...
function writeConfig(patch) {
  try {
    ...
    console.warn('[analyzer] could not persist config:', e.message);
  }
}
...
const TUNING_KEYS = ['threads', 'threadsBatch', 'gpuLayers', 'ubatch', 'kvCache'];

let _tuning = null;

function normalizeTuningValue(key, val) {
...
function resolveTuning() {
  ...
  for (const k of TUNING_KEYS) {
    effective[k] = overrides[k] === 'auto' ? auto[k] : overrides[k];
  }
...
function tuningKey(effective) {
  return TUNING_KEYS.map((k) => effective[k]).join('|');
}
...
function getModelDir(id = getSelectedModelId()) {
  return path.join(getModelsRoot(), id);
}

function modelPaths(id = getSelectedModelId()) {
  const m = MODELS[id];
...
function migrateLegacyLayout() {
  ...
  try {
    ...
    console.warn('[analyzer] model layout migration skipped:', e.message);
  }
}
...
function deleteModelFiles(id) {
  ...
  try {
    ...
    console.warn('[analyzer] could not delete model files:', e.message);
  }
}
    ...
    for (const j of jobs) {
      if (!fs.existsSync(j.dest)) {
        await downloadFile(
          j.url,
          j.dest,
          (f) => onProgress?.(base + f * j.weight, j.label),
          signal,
          {
...
let server = null; // { child, port, ready: Promise<void> }
let idleTimer = null;
let serverStarting = null; // memoized in-flight ensureServer() promise (de-dupes concurrent spawns)
...
let spawningChild = null; // the child currently mid-spawn, or null
let spawningModelId = null; // model id that mid-spawn child is loading, or null
...
function buildLlamaArgs(model, mmproj, port, concurrency, tuning, opts = {}) {
  const args = [
    '--model',
    model,
    '--mmproj',
...
async function attemptLlamaSpawn(bin, args, port) {
  ...
  const outcome = await Promise.race([
    ...
    exited,
    new Promise((resolve) => child.on('error', (e) => resolve({ kind: 'spawn', err: e }))),
  ]);
...
let onVariantFallback = null;
function setVariantFallbackHandler(fn) {
...
async function ensureServer() {
  ...
  serverStarting = (async () => {
    ...
    try {
      ...
      const code = last && last.detail && last.detail.code;
...
async function probeDuration(ffmpeg, file, signal) {
...
async function extractFrames(videoPath, signal, onProgress) {
...
async function scaleImageToDataUrl(ffmpeg, file, signal) {
...
async function scaleTopToDataUrl(ffmpeg, file, signal) {
  const safeFile = safeInputPath(file);
 ...
 */
async function assessScreenshot(imagePath, { signal } = {}) {
...
async function collectVisualInputs(post, signal, onProgress) {
  ...
  if (post.platform === 'web' && Array.isArray(post.media)) {
    const ROLE_ORDER = { hero: 0, interna: 1, footer: 2, mobile: 3 };
    const ffmpeg = resolveFfmpeg();
    const slides = post.media
      .filter(
        (m) => m && m.localPath && (m.type === 'image' || !m.type) && fs.existsSync(m.localPath),
      )
...
function buildUserPrompt(caption, frequentTags, hasFrames = true, kind = 'social') {
  ...
  const tags = Array.isArray(frequentTags)
    ? frequentTags.filter((t) => typeof t === 'string' && t.trim()).slice(0, 30)
    : [];
...
function buildWebUserPrompt(caption, frequentTags, hasFrames = true) {
  const clean = typeof caption === 'string' ? stripPromptMarkers(caption).trim() : '';
...
function cleanStringArray(arr, { keepCase = false, cap = Infinity } = {}) {
...
async function runInference(frameUrls, caption, frequentTags, signal, onToken, kind = 'social') {
  inflightCount++;
  if (idleTimer) {
    clearTimeout(idleTimer);
    idleTimer = null;
...
async function analyzeFrames(
  ...
  try {
    ...
    const isAbort =
      err?.name === 'AbortError' || err?.message === 'AbortError' || (signal && signal.aborted);
    ...
    console.warn(
      `[analyzer] inference failed, ${inflightCount === 0 ? 'restarting server and ' : ''}retrying once: ${err?.message || err}`,
    );
...
function intersectWithVocab(candidates, realTagSet) {
  const out = [];
  const seen = new Set(); // norm canonici già emessi
  const accept = (norm, form) => {
...
function expandQueryToVocab(query, { limit = SPECIFIC_VOCAB_LIMIT } = {}) {
...
async function expandSearchQuery(query, signal) {
...
function buildRefinePrompt(group) {
...
function recoverTruncatedGroups(content) {
  const objs = [];
  const stack = [];
...
function parseRefineResponse(content) {
...
function validateRefinedGroups(inputTags, parsed) {
...
async function refineOneGroup(group, signal) {
  inflightCount++;
  if (idleTimer) {
    clearTimeout(idleTimer);
    idleTimer = null;
...
async function refineTagGroups(candidates, { signal, onProgress } = {}) {
  ...
  for (const group of candidates || []) {
    ...
    try {
      ...
      console.warn(`[analyzer] cluster refine failed for a group: ${err?.message || err}`);
...
async function clusterTags({ signal, onProgress } = {}) {
  if (!getModelStatus().ready) throw new Error('MODEL_NOT_READY');
  if (idleTimer) {
    clearTimeout(idleTimer);
    idleTimer = null;
...
function buildAliasPrompt(batch, vocab) {
...
function validateAliasPairs(batch, vocab, parsed) {
  ...
  const out = [];
  const seen = new Set();
  for (const p of Array.isArray(parsed?.aliases) ? parsed.aliases : []) {
...
async function buildAliasesForBatch(batch, vocab, signal) {
  inflightCount++;
  if (idleTimer) {
    clearTimeout(idleTimer);
    idleTimer = null;
...
async function buildTagAliases({ signal, onProgress } = {}) {
  if (!getModelStatus().ready) throw new Error('MODEL_NOT_READY');
  if (idleTimer) {
    clearTimeout(idleTimer);
    idleTimer = null;
  ...
  const pairs = [];
  const usedAlias = new Set(); // un alias non deve comparire in più batch-output
  let done = 0;
  ...
  try {
    for (const batch of batches) {
      ...
      try {
        ...
        console.warn(`[analyzer] alias build failed for a batch: ${err?.message || err}`);
      }
      onProgress?.({ done: ++done, total });
    }
  } finally {
...
function buildChatSystemPrompt(broadVocab, specificVocab, activeTags) {
  const broad = Array.isArray(broadVocab)
    ? broadVocab.filter((t) => typeof t === 'string' && t.trim())
    : [];
  const specific = Array.isArray(specificVocab)
    ? specificVocab.filter((t) => typeof t === 'string' && t.trim())
    : [];
  const active = Array.isArray(activeTags)
    ? activeTags.filter((t) => typeof t === 'string' && t.trim())
    : [];
  return [
...
function retrieveSpecificTags(query, excludeSet) {
  const out = [];
  const seen = new Set(); // lowercased keys
  const push = (tag) => {
  ...
  let distinct = [];
  try {
    distinct = db.getTagDistinctivenessForTextQuery(query, { limit: SPECIFIC_VOCAB_LIMIT }) || [];
  } catch {
    distinct = [];
  }
  // Map of lowercased tag-name → topic lift, to validate lexical matches below.
  const liftByTag = new Map();
  ...
  let matches = [];
  try {
    matches = db.searchTagsByText(query, { limit: SPECIFIC_VOCAB_LIMIT }) || [];
  } catch {
    matches = [];
  }
...
function parseTagBlock(text, open, close, vocabSet) {
...
function parseKeywordBlock(text, open, close) {
  if (typeof text !== 'string') return [];
  const start = text.indexOf(open);
  if (start === -1) return [];
  const from = start + open.length;
...
function deterministicKeywords(lastUserMessage) {
  let terms = [];
  try {
...
function deterministicTagMatches(lastUserMessage, activeTags, broadVocab) {
...
async function chatSearch(messages, activeTags = [], onToken, signal) {
...
function retrieveKeywords(query, { limit = 12 } = {}) {
  try {
...
const jobsMap = new Map(); // key → serializable job record
const postCache = new Map(); // key → post (for retry)
const abortMap = new Map(); // key → AbortController
const pendingQueue = [];
...
function setJob(job, { persist = true } = {}) {
...
async function runJob(key) {
  ...
  try {
    if (!getModelStatus().ready) throw new Error('MODEL_NOT_READY');
    const frames = await collectVisualInputs(post, ac.signal, ({ stage, frac } = {}) => {
    ...
    const kind = post.platform === 'web' ? 'web' : 'social';
    const freq =
      kind === 'web' ? (Array.isArray(post.webTech) ? post.webTech : []) : db.getFrequentTags();
...
function canAnalyze(post) {
  if (!post) return false;
  const candidates = [post.videoPath, post.imagePath, post.thumbnailPath];
...
function enqueuePost(post, { pump = true } = {}) {
...
function recover() {
  try {
    ...
    console.warn('[analyzer] clearStuckAnalyzing failed:', e?.message);
  }
