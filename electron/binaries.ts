...
import { readFeedUrl, extractArchive } from './archive-utils';
import { sha256File } from './serverUtils';
import * as hardware from './hardware';

// The llama.cpp acceleration build this app knows how to provision/spawn.
type LlamaVariant = 'cpu' | 'cuda' | 'vulkan' | 'metal';

// Persisted shape of llama-variant.json (see readVariantState/writeVariantState).
interface VariantState {
  variant: LlamaVariant | null;
  explicit: boolean;
  failed: LlamaVariant[];
}

// Full variant state surfaced to the UI (chosen + effective + recommended).
interface VariantStateFull extends VariantState {
  effective: LlamaVariant;
  recommended: LlamaVariant;
}

// Per-binary presence report returned by status().
...
// Progress callback: (phase label, 0..1 fraction).
type ProgressFn = (phase: string, fraction: number) => void;

// One entry of the macOS/Linux binary-pack manifest (binaries.json).
interface ManifestPack {
  url?: string;
  sha512?: string;
}
interface Manifest {
  packs?: Record<string, ManifestPack>;
}

const MANIFEST_FILE = 'binaries.json';
const VARIANT_FILE = 'llama-variant.json';
const VALID_VARIANTS: ReadonlySet<string> = new Set(['cpu', 'cuda', 'vulkan', 'metal']);

// Narrowing guard so values read from disk / IPC can be checked against the
// LlamaVariant union without an unsafe cast.
function isLlamaVariant(v: unknown): v is LlamaVariant {
...
const LLAMA_LINUX_ASSET: Record<string, string> = {
  cpu: 'ubuntu-x64',
...
const BINARY_SHA256: Record<string, string | null> = {
...
async function verifyPinnedSha256(file: string, key: string, label: string): Promise<void> {
...
function expectedBinaries(): Record<string, string> {
  return {
...
function defaultVariant(): LlamaVariant {
  if (process.platform === 'darwin') return 'metal';
  try {
    const rec = hardware.detect().recommendedVariant;
    return isLlamaVariant(rec) ? rec : 'cpu';
  } catch {
    return 'cpu';
  }
}

function readVariantState(): VariantState {
  try {
    const s: unknown =
      JSON.parse(fs.readFileSync(path.join(app.getPath('userData'), VARIANT_FILE), 'utf8')) || {};
    const obj = (s ?? {}) as { variant?: unknown; explicit?: unknown; failed?: unknown };
    return {
      variant: isLlamaVariant(obj.variant) ? obj.variant : null,
      explicit: !!obj.explicit,
      failed: Array.isArray(obj.failed) ? obj.failed.filter(isLlamaVariant) : [],
    };
  } catch {
...
function writeVariantState(patch: Partial<VariantState>): VariantState {
...
function getLlamaVariant(): LlamaVariant {
...
function getVariantState(): VariantStateFull {
  const st = readVariantState();
...
function setLlamaVariant(variant: unknown): LlamaVariant {
  const v = isLlamaVariant(variant) ? variant : defaultVariant();
  const st = readVariantState();
...
function markVariantFailed(variant: unknown): VariantStateFull {
  if (!isLlamaVariant(variant) || variant === 'cpu') return getVariantState();
  const st = readVariantState();
...
function llamaBuildMarker(): string {
  return path.join(runtimeBinDir(), 'llama', '.build');
}
function installedLlamaBuild(): string | null {
  try {
...
function writeLlamaBuildMarker(): void {
  try {
    fs.writeFileSync(llamaBuildMarker(), LLAMA_BUILD);
  } catch (err) {
    console.warn('[binaries] could not write llama build marker:', (err as Error).message);
  }
}
...
function llamaLibsResolvable(): boolean {
...
function repairLlamaSymlinks(): void {
  const dir = path.join(runtimeBinDir(), 'llama');
  let entries: fs.Dirent[];
  try {
    entries = fs.readdirSync(dir, { withFileTypes: true });
  ...
  for (const e of entries) {
    ...
    try {
      ...
      console.warn(
        `[binaries] could not repair llama symlink ${e.name}: ${(err as Error).message}`,
      );
    }
  }
}
...
async function downloadWithRetry(
  ...
  for (let attempt = 1; attempt <= attempts; attempt++) {
    try {
      ...
      if (attempt < attempts) {
        console.warn(
          `[binaries] download failed (attempt ${attempt}/${attempts}) for ${url}: ${(err as Error)?.message || err}`,
        );
...
function freshTmpDir(tag: string): string {
...
// always pass through.
function perPercent(emit: ProgressFn): ProgressFn {
  let lastPhase: string | null = null;
  let lastPct = -1;
  return (phase: string, fraction: number) => {
...
async function ensureBinariesWindows(
  ...
  const has = (p: string): boolean => !force && fs.existsSync(p);
  const ghLlama = (asset: string): string =>
  ...
  const installedVariant = ((): string | null => {
    try {
...
function packFeedCandidates(): string[] {
  const stable = readFeedUrl();
  if (!stable) return [];
  let channel = 'stable';
  try {
    const p = path.join(app.getPath('userData'), 'update-channel.json');
    if ((JSON.parse(fs.readFileSync(p, 'utf8')) as { channel?: string }).channel === 'beta')
      channel = 'beta';
  } catch {
    /* default stable */
  }
...
async function fetchManifestWithFeed(): Promise<{ feed: string; manifest: Manifest }> {
...
async function ensureBinariesMac(
  onProgress: ProgressFn,
  force: boolean,
): Promise<{ ok: true; installed: string }> {
...
async function ensureBinariesLinux(
  onProgress: ProgressFn,
  force: boolean,
): Promise<{ ok: true; installed: string }> {
  if (process.arch !== 'x64') {
...
// Result of an ensureBinaries pass: a skip/short-circuit shape, or the per-platform
// installer's { ok, installed } success report.
type EnsureResult =
...
let _lastPromise: Promise<EnsureResult> | null = null;
