...
function buildTagCommunities(
  freq,
  edges,
  { minJaccard = 0.15, maxGroupSize = 14, iterations = 6, cosSim = null, alpha = 0.5 } = {},
) {
  // Peso d'arco fuso jaccard+cos (o solo-jaccard se cosSim assente).
  const edgeWeight = (a, b, c) => {
  ...
  const propagate = (nodeSet, weightedEdges, thr) => {
    const adj = new Map();
    const link = (a, b, w) => {
  ...
  const total = (g) => g.reduce((s, t) => s + (freq.get(t) || 0), 0);
  groups.sort((A, B) => total(B) - total(A));
  return groups;
}

module.exports = { buildTagCommunities, cosineSim };
