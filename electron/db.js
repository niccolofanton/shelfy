...
const fs = require('fs');
const { normalizeExportedPost: normalizeIg, igDateFromShortcode } = require('./ig-parser');
const { normalizeExportedPost: normalizeTw } = require('./tw-parser');
...
function wordMatchFn(haystack, needle) {
...
function registerSqlFunctions(database) {
  database.function('word_match', { deterministic: true }, wordMatchFn);
}
...
function migrate(db) {
  ...
  const collectionCols = { platform: 'TEXT', external_id: 'TEXT', ig_name: 'TEXT' };
  ...
  const ptCols = db.prepare('PRAGMA table_info(post_tags)').all();
  ...
  const aliasCols = db.prepare('PRAGMA table_info(tag_alias)').all();
...
function backfillInstagramTimestamps(db) {
  const rows = db
    .prepare(
...
function backfillDerivedTags(db) {
  const tagPosts = db
    .prepare(
      `
...
function rowToPost(row) {
  if (!row) return null;
  const webMeta = parseJson(row.web_meta_json, null);
  return {
    id: row.id,
    ...
    webPalette: parseJson(row.web_palette_json, []),
    webFonts: parseJson(row.web_fonts_json, []),
    webTech: parseJson(row.web_tech_json, []),
    webAwards: parseJson(row.web_awards_json, []),
    webPages: parseJson(row.web_pages_json, []),
    webMeta,
...
function normalizeTagRows(raw) {
  const arr = Array.isArray(raw) ? raw : parseTags(raw);
  const out = [];
  const seen = new Set();
  for (const t of arr) {
...
function deriveMedia(post) {
  ...
  if (url && (post.mediaType ?? post.media_type) !== 'text') {
    return [{ type: (post.mediaType ?? post.media_type) === 'video' ? 'video' : 'image', url }];
  }
  return [];
}
...
function chunkedInQuery(ids, build, batch = 500) {
  const out = [];
  for (let i = 0; i < ids.length; i += batch) {
    const chunk = ids.slice(i, i + batch);
    const placeholders = chunk.map(() => '?').join(',');
    out.push(...db.prepare(build(placeholders)).all(...chunk));
  }
  return out;
}
...
function attachCollections(posts) {
  if (!posts.length) return posts;
  const ids = posts.map((p) => p.id);
  const rows = chunkedInQuery(
    ids,
...
const SHORT_CONTENT_TERMS = new Set([
  '3d',
  '2d',
  'ai',
  'ar',
...
function contentTermsOrRaw(query, opts = {}) {
...
function extractContentTerms(query, { minLen = 3 } = {}) {
  return [
    ...new Set(
      String(query ?? '')
        .toLowerCase()
...
function buildPostFilter(filters = {}) {
  ...
  const buildTagScoreUnit = (norm, w) => {
...
const _termIdfCache = new Map();
function termIdfWeights(terms, cols) {
  const out = {};
  ...
  try {
    ...
    const row = db
      ...
      .get(...misses.flatMap((t) => cols.map(() => `%${likeEscape(t)}%`)));
    const N = row.n || 0;
    if (!N) {
...
function tagIdfWeights(norms) {
  ...
  try {
    const N = db.prepare('SELECT COUNT(*) AS c FROM posts').get().c || 0;
    if (!N) {
...
function recencyOrder(sortOrder) {
...
function getPosts(filters = {}) {
  ...
  return { posts: attachCollections(attachMedia(rows.map(rowToPost))), total };
}
...
function getPostsByIds(ids = []) {
  ...
  for (let i = 0; i < ids.length; i += BATCH) {
    const chunk = ids.slice(i, i + BATCH);
    const placeholders = chunk.map(() => '?').join(',');
    const rows = db.prepare(`SELECT * FROM posts WHERE id IN (${placeholders})`).all(...chunk);
...
function existingIds(ids = []) {
  ...
  for (let i = 0; i < ids.length; i += BATCH) {
    ...
    const placeholders = chunk.map(() => '?').join(',');
    const rows = db.prepare(`SELECT id FROM posts WHERE id IN (${placeholders})`).all(...chunk);
...
//   - otherwise → all posts.
// No forced limit.
function getPostsForAnalysis({ ids = null, missingOnly = false } = {}) {
  if (!db) throw new Error('Database not initialized');
  ...
  const rows = db.prepare(`SELECT ${COLS} FROM posts ${where}`).all();
...
function replacePostMedia(postId, media) {
...
function mergePostMedia(postId, media) {
  const ins = db.prepare(
    'INSERT OR IGNORE INTO post_media (post_id, position, media_type, source_url, local_path) VALUES (?, ?, ?, ?, ?)',
  );
...
function bulkUpsert(posts, { overwriteAi = false } = {}) {
  if (!db) throw new Error('Database not initialized');
  ...
  const insertMany = db.transaction((postsArr) => {
    for (const post of postsArr) {
      ...
      if (result.changes === 0) {
        ...
        if (aiFields) {
          const cur = existingStatus.get(post.id);
...
function upsertWebReference(ref, { overwriteAi = true } = {}) {
  if (!db) throw new Error('Database not initialized');
...
function upsertWebReferences(refs = [], { overwriteAi = true } = {}) {
  if (!db) throw new Error('Database not initialized');
...
function createWebPlaceholder(url) {
  if (!db) throw new Error('Database not initialized');
  const finalUrl = url;
...
function getStats() {
  ...
  const platformRows = db
    ...
    .all();
  const byPlatform = { instagram: 0, twitter: 0, pinterest: 0, web: 0 };
...
function updatePaths(id, { thumbnailPath, imagePath, videoPath, thumbBlur }) {
  if (!db) throw new Error('Database not initialized');
  db.prepare(
    `
    UPDATE posts SET
...
function listPostsMissingThumbBlur() {
  if (!db) throw new Error('Database not initialized');
  return db
    .prepare(
      `
...
function setThumbBlur(id, thumbBlur) {
  if (!db) throw new Error('Database not initialized');
...
function updateMediaPath(postId, position, localPath, mediaType = 'image') {
  if (!db) throw new Error('Database not initialized');
  const res = db
...
function serializeArrayField(v) {
  if (v === null) return null;
...
function updateAiAnalysis(id, fields = {}) {
  if (!db) throw new Error('Database not initialized');
...
function updateUserContent(id, fields = {}) {
  if (!db) throw new Error('Database not initialized');
...
function clearAiDescriptions(ids = []) {
  if (!db) throw new Error('Database not initialized');
...
function applyAiAnalysis(id, fields = {}) {
  const {
    description,
    tags,
    status,
...
function getFrequentTags(limit = 30) {
  if (!db) throw new Error('Database not initialized');
  ...
  const formRows = db
    ...
    .all(...norms);
  const forms = new Map(); // norm → Map(form → count)
...
function getAiOverview() {
  ...
  const analyzed = db
    .prepare("SELECT COUNT(*) as count FROM posts WHERE ai_status = 'done'")
    .get().count;
  ...
  const uniqueTags = db
    .prepare('SELECT COUNT(DISTINCT tag_norm) AS count FROM post_tags')
    .get().count;
  const taggedPosts = db
    .prepare('SELECT COUNT(DISTINCT post_id) AS count FROM post_tags')
    .get().count;
...
function getTagStats({ limit = 300, tier = null } = {}) {
  if (!db) throw new Error('Database not initialized');
  ...
  const tierParam = tierWhere ? [tier] : [];
  ...
  const catRows = db
    ...
    .all(...norms);
  const cats = new Map(); // norm → [{category, count}]
...
function getEntityStats({ limit = 300 } = {}) {
  if (!db) throw new Error('Database not initialized');
  const top = db
    .prepare(
      `
...
function getTagCooccurrence(tag, { limit = 8 } = {}) {
  if (!db) throw new Error('Database not initialized');
...
function searchTagsByText(query, { limit = 40, minTermLen = 3 } = {}) {
  if (!db) throw new Error('Database not initialized');
...
function getTopTagsForTextQuery(query, { limit = 40, postLimit = 80, minTermLen = 3 } = {}) {
  if (!db) throw new Error('Database not initialized');
  const terms = contentTermsOrRaw(query, { minLen: minTermLen });
  if (!terms.length) return [];
...
let _globalTagCounts = null;
function getGlobalTagCounts() {
...
function getTagDistinctivenessForTextQuery(
  ...
  const pw = new Map(); // postId → Σ idf of matched terms
  const pBest = new Map(); // postId → max idf of any matched term
  ...
  for (const r of matchStmt.iterate(...termLikes, ...termLikes)) {
    for (let i = 0; i < terms.length; i++) {
      if (remaining[i] <= 0 || !r[`m${i}`]) continue;
      remaining[i] -= 1;
      if (remaining[i] === 0) openCaps -= 1;
      const w = idf.get(terms[i]);
      pw.set(r.id, (pw.get(r.id) || 0) + w);
      if (w > (pBest.get(r.id) || 0)) pBest.set(r.id, w);
    }
    if (!openCaps) break;
  }
  if (!pw.size) return [];
...
function getKeywordsForTextQuery(query, { limit = 12, postLimit = 600, minTermLen = 3 } = {}) {
  if (!db) throw new Error('Database not initialized');
  const terms = contentTermsOrRaw(query, { minLen: minTermLen });
  if (!terms.length) return [];
  ...
  const phraseCount = new Map(); // norm phrase → count in matched set
  const phraseForm = new Map(); // norm phrase → display form (first seen, trimmed)
  const setTokenCount = new Map(); // token → count in matched set
  for (const row of rows) {
    let arr;
    try {
  ...
  const idf = (tok) => Math.log((globalTotal + 1) / ((globalTokenCount.get(tok) || 0) + 1));
  const scorePhrase = (norm) => {
...
let _globalKwTokenCounts = null;
function getGlobalKeywordTokenCounts() {
  if (_globalKwTokenCounts) return _globalKwTokenCounts;
  const m = new Map();
  const rows = db.prepare('SELECT ai_keywords FROM posts WHERE ai_keywords IS NOT NULL').all();
  for (const row of rows) {
    let arr;
    try {
      arr = JSON.parse(row.ai_keywords);
...
function invalidateGlobalCaches() {
...
function getUnaliasedTags({ limit = 300 } = {}) {
  if (!db) throw new Error('Database not initialized');
  const rows = db
    .prepare(
      `
...
function getCanonicalVocab({ limit = 300 } = {}) {
  if (!db) throw new Error('Database not initialized');
  const rows = db
    .prepare(
      `
...
function saveTagAliases(pairs = [], { status = 'proposed' } = {}) {
  if (!db) throw new Error('Database not initialized');
  ...
  const tx = db.transaction(() => {
    ...
    const rootOf = (norm, fallbackForm) => {
      let cur = norm;
...
function getTagAliases({ status = null } = {}) {
  if (!db) throw new Error('Database not initialized');
...
function setAliasStatus(aliasNorm, status) {
  if (!db) throw new Error('Database not initialized');
...
function runClusterWorker(payload) {
...
async function clusterTagCommunities(freq, edges, vecByNorm, opts) {
  try {
    ...
    console.log(`[db] cluster worker non disponibile (${err?.message || err}): calcolo in-process`);
...
async function getTagCandidateGroups({
  ...
  const formByNorm = new Map();
  {
    const norms = [...freq.keys()];
    const formRows = chunkedInQuery(
      norms,
      (ph) =>
  ...
  try {
    ...
    console.log(
      `[db] embeddings non disponibili (${err?.message || err}): clustering con solo-jaccard`,
    );
  }
  ...
  const topNeighbors = (n) =>
    (nbr.get(n) || [])
      .sort((x, y) => y[1] - x[1])
      .slice(0, neighborLimit)
      .map(([t]) => t);

  const groups = communities.map((tags) => {
    const neighbors = {};
  ...
  if (leftover.length) {
    ...
    const groupOf = new Map(); // norm → indice gruppo
...
function saveClusterRun(clusters = []) {
  if (!db) throw new Error('Database not initialized');
...
function getTagClusters({ maxClusters } = {}) {
  if (!db) throw new Error('Database not initialized');
  ...
  const byCluster = new Map(); // id → [norm]
  for (const r of db
  ...
  const displayForm = (norm) => {
    if (formCache.has(norm)) return formCache.get(norm);
    const fm = new Map();
    for (const r of formStmt.all(norm)) fm.set(r.form, r.c);
  ...
  return Number.isFinite(maxClusters) && maxClusters > 0 ? out.slice(0, maxClusters) : out;
}
...
function setClusterStatus(id, status) {
  if (!db) throw new Error('Database not initialized');
...
function renameCluster(id, label) {
  if (!db) throw new Error('Database not initialized');
...
function removeTagFromCluster(tagNorm, clusterId) {
  if (!db) throw new Error('Database not initialized');
...
function getTagMergeSuggestions({ limit = 30 } = {}) {
  if (!db) throw new Error('Database not initialized');
  ...
  const bucketKey = (band) => `${band}`;
  const buckets = new Map(); // key → [index]
  tags.forEach((t, i) => {
  ...
  const compared = new Set(); // "i:j" dedupe across overlapping neighbor scans
  tags.forEach((t, i) => {
    if (!t.norm) return;
    const band = Math.floor(t.norm.length / BAND);
...
function getTagHealth() {
  ...
  const orphanTags = orphanNorms.map((norm) => {
    const fm = new Map();
    for (const r of formStmt.all(norm)) fm.set(r.form, r.c);
...
function rewritePostTags(rawTags, fromKeys, to) {
  ...
  const tx = db.transaction(() => {
    for (const row of rows) {
      ...
      if (next) {
        ...
        const tierByNorm = new Map();
        for (const r of selTier.all(row.id)) tierByNorm.set(r.norm, r.tier ?? null);
        let mergedTier;
...
function getTagGraph({ maxNodes = 60, minEdgeWeight = 2 } = {}) {
  const topNodes = db
    .prepare(
  ...
  const edges = db
    ...
    .all(...nodeIds, ...nodeIds, minEdgeWeight);
...
function getPostIdsByTags(tags = [], mode = 'or') {
  if (!db) throw new Error('Database not initialized');
  const wanted = tags
    .map((t) =>
      String(t ?? '')
  ...
  if (mode === 'and') {
    ...
    for (const tag of wanted) {
      const set = new Set(
        db
          .prepare('SELECT post_id FROM post_tags WHERE tag_norm = ?')
          .all(tag)
          .map((r) => r.post_id),
      );
      acc = acc === null ? set : new Set([...acc].filter((id) => set.has(id)));
      if (!acc.size) return [];
    }
    return [...acc];
  }
...
function searchPostsByTags(
  tags = [],
  { mode = 'or', limit = 60, offset = 0, source = 'all' } = {},
) {
  if (!db) throw new Error('Database not initialized');
...
function normalizeImportedPost(post) {
  if (post.platform === 'instagram' || post.shortcode) return normalizeIg(post);
  return normalizeTw(post);
}
...
async function importFromJSON(filePath) {
  ...
  const collectionDefs =
    parsed && !Array.isArray(parsed) && Array.isArray(parsed.collections) ? parsed.collections : [];

  let normalized;
  try {
    normalized = posts.map(normalizeImportedPost);
  } catch (err) {
    throw new Error(`Failed to normalize posts: ${err.message}`);
  }
...
function getCollectionsForExport() {
  if (!db) throw new Error('Database not initialized');
  return db
    .prepare(
...
function attachCollectionKeys(posts) {
  if (!db || !posts.length) return posts;
  const ids = posts.map((p) => p.id);
  const rows = chunkedInQuery(
    ids,
...
function exportAllPosts(platforms) {
  if (!db) throw new Error('Database not initialized');
  ...
  const rows = db.prepare(sql).all(...params);
  return attachCollectionKeys(attachTagTiers(attachMedia(rows.map(rowToPost))));
}
...
function findOrCreateCollection(def) {
  const ext =
    def.externalId != null && String(def.externalId) !== '' ? String(def.externalId) : null;
  let row = null;
  if (ext != null) row = db.prepare('SELECT id FROM collections WHERE external_id = ?').get(ext);
  if (!row && def.name)
    row = db
      .prepare('SELECT id FROM collections WHERE name = ? AND external_id IS NULL')
      .get(String(def.name).trim());
...
function importCollections(defs = [], rawPosts = []) {
  if (
    !db ||
    (!defs.length &&
  ...
  const tx = db.transaction(() => {
    ...
    const byCol = new Map(); // localId → Set(postId)
    ...
    importCollections._lastLinks = links;
  });
  tx();
  return { collectionsCreated: created, linksAdded: importCollections._lastLinks || 0 };
}
...
function createCollection({ name, color, platform, externalId, igName } = {}) {
  if (!db) throw new Error('Database not initialized');
...
function updateCollection(id, { name, color } = {}) {
  if (!db) throw new Error('Database not initialized');
  db.prepare(
    `
...
function addPostsToCollections(postIds = [], collectionIds = []) {
  if (!db) throw new Error('Database not initialized');
  const ins = db.prepare(
...
function removePostFromCollection(postId, collectionId) {
  if (!db) throw new Error('Database not initialized');
...
function jobDelete(kind, key) {
  if (!db) throw new Error('Database not initialized');
  db.prepare('DELETE FROM jobs WHERE kind = ? AND key = ?').run(kind, key);
}

function jobsDeleteMany(kind, keys = []) {
  if (!db) throw new Error('Database not initialized');
...
function jobDeleteAll(kind) {
  if (!db) throw new Error('Database not initialized');
  db.prepare('DELETE FROM jobs WHERE kind = ?').run(kind);
}

function jobsByKind(kind) {
  if (!db) throw new Error('Database not initialized');
  return db.prepare('SELECT * FROM jobs WHERE kind = ? ORDER BY created_at, rowid').all(kind);
}
...
function getCurrentCaptureFilePaths(postId) {
  if (!db) throw new Error('Database not initialized');
  const row = db
  ...
  const slides = db
    ...
    .all(postId);
  const paths = [row?.thumbnail_path, row?.image_path, row?.video_path];
...
function getLocalFilePaths(postId) {
  if (!db) throw new Error('Database not initialized');
...
function snapshotPagePaths(webPagesJson) {
  const pages = parseJson(webPagesJson, []);
...
function archiveCurrentWebSnapshot(postId) {
  if (!db) throw new Error('Database not initialized');
  const row = db
    .prepare(
  ...
  const pages = parseJson(row.web_pages_json, []);
...
function getWebSnapshots(postId) {
  if (!db) throw new Error('Database not initialized');
  const rows = db
  ...
  return rows.map((r) => ({
    ...
    title: r.title ?? null,
    webPages: parseJson(r.web_pages_json, []),
    webPalette: parseJson(r.web_palette_json, []),
    webFonts: parseJson(r.web_fonts_json, []),
    webTech: parseJson(r.web_tech_json, []),
    webAwards: parseJson(r.web_awards_json, []),
    webMeta: parseJson(r.web_meta_json, null),
...
function deleteWebSnapshot(id) {
  if (!db) throw new Error('Database not initialized');
  const row = db.prepare('SELECT web_pages_json FROM web_snapshots WHERE id = ?').get(id);
...
// "delete site". Deduped.
function getWebSiteFilePaths(postId) {
  if (!db) throw new Error('Database not initialized');
  const current = getCurrentCaptureFilePaths(postId);
  const snaps = db
    .prepare('SELECT web_pages_json FROM web_snapshots WHERE post_id = ?')
...
function promoteSnapshotToPost(postId, snapshotId) {
  if (!db) throw new Error('Database not initialized');
  const snap = db
    .prepare('SELECT * FROM web_snapshots WHERE id = ? AND post_id = ?')
    .get(snapshotId, postId);
  if (!snap) return false;
  const pages = parseJson(snap.web_pages_json, []);
  const hero = (Array.isArray(pages) ? pages : []).find((p) => p && p.screenshotPath) || {};
  db.prepare(
    `UPDATE posts SET
       author_name = @title,
       timestamp = @timestamp,
...
function clearWebPostToPlaceholder(postId) {
  if (!db) throw new Error('Database not initialized');
  db.prepare(
    `UPDATE posts SET
...
function deleteLatestReport(postId) {
  if (!db) throw new Error('Database not initialized');
...
function clearPostLocalFiles(postId) {
  if (!db) throw new Error('Database not initialized');
  db.prepare(
...
module.exports = {
  ...
  __backfillDerivedTags: () => backfillDerivedTags(db),
  __termIdfWeights: termIdfWeights,
};
