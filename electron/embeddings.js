...
function isModelReady(id = getSelectedModelId()) {
...
let _downloadingId = null; // id del modello il cui download è in corso
let _downloadAbort = null; // AbortController per il download in corso
let _downloadAction = null; // null | 'pause' | 'cancel' — come trattare un abort
...
let server = null; // { child, port, ready, modelId }
let serverStarting = null; // promise ensureServer() in volo (single-flight guard)
let idleTimer = null;
...
function parseEmbeddingResponse(json, expectedCount) {
...
async function embedBatch(port, texts, signal) {
