...
// Allegato così come arriva dal renderer (UNTRUSTED): la forma è ignota finché
// non viene validata da sanitizeAttachments.
interface RawAttachment {
  filename?: string;
  content?: string;
}

// Allegato normalizzato nel formato atteso da Resend.
interface SanitizedAttachment {
  filename: string;
  content: string;
}

// Payload del feedback inviato dal renderer (più la version aggiunta dal main).
...
// Esito dell'invio, restituito al renderer via IPC.
 ...
 */
function sanitizeAttachments(attachments: unknown): SanitizedAttachment[] {
  if (!Array.isArray(attachments)) return [];
  const out: SanitizedAttachment[] = [];
  let total = 0;
  for (const a of attachments as RawAttachment[]) {
...
async function sendFeedback({
  ...
  version,
  attachments,
}: FeedbackPayload = {}): Promise<FeedbackResult> {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        try {
          ...
          if (isJsonErrorBody(j) && (j.message || j.error))
            detail = (j.message || j.error) as string;
        } catch {
      ...
      return { ok: true, id: extractId(json) };
    } catch (err) {
      console.error('[feedback] relay send failed:', err);
      return { ok: false, error: errorMessage(err) || 'Errore di rete.' };
    } finally {
      clearTimeout(timer);
    }
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      try {
        const j: unknown = await res.json();
        if (isJsonErrorBody(j) && j.message) detail = j.message;
      } catch {
        /* corpo non-JSON: tieni lo status */
      }
...
// Corpo di errore JSON (relay / Resend): forma sconosciuta, narrowiamo i soli
// campi che leggiamo (message / error).
interface JsonErrorBody {
  message?: string;
  error?: string;
}

function isJsonErrorBody(j: unknown): j is JsonErrorBody {
  return typeof j === 'object' && j !== null;
}

// Estrae l'`id` dalla risposta JSON (relay / Resend), che ha forma sconosciuta.
...
// Estrae il messaggio da un errore di forma sconosciuta (err?.message).
