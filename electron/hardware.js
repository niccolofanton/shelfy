...
function probe(cmd, args, timeout = 1500) {
  try {
...
function detectCpu() {
  ...
  if (process.platform === 'darwin') {
    ...
    if (!physical) {
      physical = Number(probe('powershell', ['-NoProfile', '-Command',
        '(Get-CimInstance Win32_Processor | Measure-Object -Property NumberOfCores -Sum).Sum'], 4000)) || 0;
    }
  } else {
    ...
    try {
      ...
      for (const line of txt.split('\n')) {
        ...
        else if (line.trim() === '') { if (core) ids.add(`${pkg}:${core}`); pkg = core = ''; }
      }
      physical = ids.size;
...
function detectGpu(totalRamGB) {
  // NVIDIA first: if nvidia-smi answers we know it's CUDA-capable and get exact VRAM.
  const smi = probe('nvidia-smi', ['--query-gpu=name,memory.total', '--format=csv,noheader,nounits']);
  if (smi) {
    const [name, mib] = smi.split('\n')[0].split(',').map((s) => s.trim());
  ...
  if (process.platform === 'darwin') {
    ...
    const name = probe('sysctl', ['-n', 'machdep.cpu.brand_string']) || (isArm ? 'Apple Silicon' : 'Mac');
    return {
  ...
  if (process.platform === 'win32') {
    ...
    if (!out) {
      out = probe('powershell', ['-NoProfile', '-Command',
        '(Get-CimInstance Win32_VideoController).Name | ForEach-Object { "Name=" + $_ }'], 4000);
    }
    const names = out.split(/\r?\n/).map((l) => l.match(/Name=(.+)/)?.[1]?.trim()).filter(Boolean);
    const name = names.find((n) => /radeon|amd|arc|intel|nvidia|geforce/i.test(n)) || names[0] || '';
    const vendor = /radeon|amd/i.test(name) ? 'amd' : /arc|intel/i.test(name) ? 'intel'
      : /nvidia|geforce/i.test(name) ? 'nvidia' : 'unknown';
...
function detect() {
  ...
  const recommendedVariant = process.platform === 'darwin' ? 'metal'
    : (gpu.cuda || gpu.vendor === 'nvidia') ? 'cuda'
    : (gpu.vendor === 'amd' || gpu.vendor === 'intel') ? 'vulkan'
    : 'cpu';
...
function computeLlamaTuning({ variant = 'cpu', modelSizeGB = 0, concurrency = 1, ctxPerSlot = 4096 } = {}) {
  const hw = detect();
  ...
  const gpuLayers = gpuAccel ? 'fit' : 0;
...
function estimateKvCacheGB(totalCtx, modelSizeGB) {
...
function computeEmbeddingTuning() {
  const hw = detect();
  const baseCores = hw.appleSilicon && hw.cpu.perf ? hw.cpu.perf : hw.cpu.physical;
  return { threads: Math.max(1, Math.min(8, baseCores)) };
}
...
function recommendModel(presets) {
  const hw = detect();
  ...
  const budgetGB = hw.gpu.vramGB != null && !hw.gpu.unified
