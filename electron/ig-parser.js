...
function igDateFromShortcode(shortcode) {
 ...
 */
function findEdgeArrays(obj, results = [], depth = 0) {
...
function parseRestItems(items) {
  ...
  return items
    .map((item) => {
      ...
      const captionRaw = item.caption?.text ?? item.caption ?? '';
...
module.exports = { parseResponseBody, normalizeExportedPost, igDateFromShortcode };
