...
function assetHeaders(mime, etag) {
  const h = NO_HTTP_CACHE
...
function hardenWebContents() {
  ...
  app.on('web-contents-created', (_e, contents) => {
    ...
    contents.on('will-attach-webview', (_event, webPreferences, params) => {
      ...
      if (params && params.disablewebsecurity !== undefined) {
      ...
      const requested = webPreferences.preload || webPreferences.preloadURL;
      if (requested) {
...
function createWindow() {
  ...
  if (isDev) {
    ...
    (function loadWithRetry(retries = 10, delay = 500) {
  ...
  try {
    require('./interceptor').setupInterceptor(mainWindow);
  } catch (err) {
    console.error('[main] Failed to set up interceptor:', err);
  }

  try {
    require('./ipc').registerIpcHandlers(mainWindow);
  } catch (err) {
...
app.whenReady().then(() => {
  ...
  const allowMic = (permission) => permission === 'media' || permission === 'audioCapture';
  session.defaultSession.setPermissionRequestHandler((_wc, permission, callback, details) => {
    callback(allowMic(permission) && isAppOrigin(details && details.requestingUrl));
  });
  ...
  try {
    require('./analyzer').setVariantFallbackHandler((failedVariant) => {
...
app.on('before-quit', () => {
  ...
  try {
    require('./analyzer').forceShutdown();
  } catch {}
  try {
    require('./stt').forceShutdown();
  } catch {}
  try {
    require('./embeddings').forceShutdown();
  } catch {}
  // Abort any in-flight web capture so a hidden BrowserWindow can't linger.
  try {
    require('./weborchestrator').cancelAll();
  } catch {}
  // Close the shared Playwright browser so no headless Chromium is orphaned.
  try {
    require('./capture-engine').closeBrowser();
  } catch {}
