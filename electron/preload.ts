...
import type {
  ...
  ManualBookmarkFile,
  ManualBookmarkResult,
} from '../types/electron-api';

try {
  // Kept as require() (not top-level import) so a failed load lands in the catch
  // below and is logged as a FATAL preload error, instead of crashing the
  // renderer surface before any diagnostics can run. typeof import(...) gives
  // these real Electron/Node types without changing the runtime load path.
  ...
  const electronAPI: ElectronAPI = {
    ...
    addManualBookmark: (payload: ManualBookmarkPayload): Promise<ManualBookmarkResult> => {
      const byteLen = (v: ManualBookmarkFile['original']): number => {
        const sized = v as { byteLength?: number; length?: number } | null | undefined;
        return sized ? Number(sized.byteLength ?? sized.length) || 0 : 0;
      };
      const files: ManualBookmarkFile[] = Array.isArray(payload?.files) ? payload.files : [];
      let total = 0;
    ...
    webviewPreloadPath: (require('url') as typeof import('url'))
  ...
  console.error('[preload] FATAL ERROR:', (err as Error).message, (err as Error).stack);
}
