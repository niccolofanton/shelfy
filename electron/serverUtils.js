...
const MODEL_HOST_SUFFIX_ALLOWLIST = [
  '.huggingface.co',
  '.hf.co',
];

function isAllowedModelHost(hostname) {
...
async function waitForHttp(port, healthPath, timeout) {
  const t0 = Date.now();
  while (Date.now() - t0 < timeout) {
    try {
      ...
      const r = await fetch(`http://127.0.0.1:${port}${healthPath}`, { signal: AbortSignal.timeout(2000) });
      if (r.ok) return;
    } catch {}
...
async function downloadFile(url, dest, onProgress, signal, { keepPartialOnAbort, expectedSha } = {}) {
  const tmp = `${dest}.part`;

  // Resume from a prior partial download when one is present.
  let startByte = 0;
  try { startByte = fs.statSync(tmp).size; } catch { startByte = 0; }
  ...
  try {
    for await (const chunk of res.body) {
      ...
      if (!out.write(chunk)) {
        await new Promise((resolve, reject) => {
          ...
          const onDrain = () => { out.removeListener('error', onError); resolve(); };
          const onError = (e) => { out.removeListener('drain', onDrain); reject(e); };
    ...
    if (!total) {
      ...
      console.warn(`[serverUtils] no content-length for ${path.basename(dest)}: cannot validate size (${received} bytes received)`);
    }
    ...
    if (expectedSha) {
      ...
      if (actual.toLowerCase() !== String(expectedSha).toLowerCase()) {
        throw new Error(`SHA256 mismatch for ${path.basename(dest)}: expected ${expectedSha}, got ${actual}`);
      }
    }
...
module.exports = { firstExisting, freePort, waitForHttp, downloadFile, isAllowedModelHost, sha256File };
