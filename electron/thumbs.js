...
function thumbCacheEntry(resolved, stat, width, ext) {
  const png = ext === 'png';
  const key = crypto
    .createHash('sha1')
...
async function generateThumb(resolved, width, png) {
...
async function thumbnailFor(resolved, width, ext) {
  let stat;
  try {
  ...
  if (inflightThumbs.has(cachePath)) return inflightThumbs.get(cachePath);
  const job = (async () => {
    await takeThumbSlot();
    try {
      const data = await generateThumb(resolved, width, png);
...
async function thumbETag(resolved, width, ext) {
  try {
...
async function prewarmThumbCache() {
  let generated = 0;
...
async function microThumbDataUri(sourcePath) {
  try {
...
async function backfillThumbBlurs() {
  let rows;
  try {
    rows = db.listPostsMissingThumbBlur();
  } catch (err) {
    console.warn(`[thumb-blur] backfill scan failed: ${err?.message || err}`);
    return 0;
  }
  let written = 0;
