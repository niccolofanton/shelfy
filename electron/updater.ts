...
// Updater status surfaced to the renderer; drives the toast and Settings affordances.
type UpdateStatus =
  | 'idle'
  | 'available'
  | 'downloading'
...
// Shape of the in-memory updater state shared with the renderer.
...
// The untrusted source.json manifest (Windows self-rebuild path).
...
let _pendingManifest: SourceManifest | null = null; // Windows: the source.json we're tracking for rebuild
...
function notifyUpdateReady(version: string | null): void {
  try {
...
// ── Channel persistence ──────────────────────────────────────────────────────
const VALID_CHANNELS = new Set<string>(['stable', 'beta']);
...
let _channel: UpdateChannel | null = null;

function getUpdateChannel(): UpdateChannel {
  if (_channel) return _channel;
  try {
    const { channel } = JSON.parse(fs.readFileSync(channelFilePath(), 'utf8')) as {
      channel?: string;
    };
    _channel = VALID_CHANNELS.has(channel ?? '') ? (channel as UpdateChannel) : 'stable';
  } catch {
    _channel = 'stable';
  }
  return _channel;
...
function setUpdateChannel(channel: string | undefined): UpdateChannel {
  const prev = getUpdateChannel();
  const ch: UpdateChannel = VALID_CHANNELS.has(channel ?? '')
...
function manifestChannel(channel: UpdateChannel): string {
  return channel === 'beta' ? 'beta' : 'latest';
}

// Source manifest filename for the self-rebuild (Windows) path.
function sourceManifestName(channel: UpdateChannel): string {
...
function feedForChannel(channel: UpdateChannel): string | null {
...
function isNewer(remote: string, local: string): boolean {
  ...
  const cmpPre = (a: string, b: string): number => {
    if (a === b) return 0;
...
function ymlSha512For(
  yml: string | null | undefined,
  name: string | null | undefined,
): string | null {
  if (!yml || !name) return null;
  const target = String(name).trim();
  const isB64Sha512 = (s: unknown): boolean =>
    typeof s === 'string' && /^[A-Za-z0-9+/]{80,}={0,2}$/.test(s);
...
function perPercent(emit: (f: number) => void): (f: number) => void {
  let last = -1;
  return (f: number) => {
...
async function checkMacUpdate(channel: UpdateChannel): Promise<void> {
...
async function checkLinuxUpdate(channel: UpdateChannel): Promise<void> {
  const feed = feedForChannel(channel);
  if (!feed) return;
  const mc = manifestChannel(channel);
...
function nodeMajor(nodeExe: string): number {
  try {
...
function findBuildNodeDir(): string | null {
...
function cleanupOldRebuilds(keepVersion: string): void {
...
async function checkWindowsUpdate(channel: UpdateChannel): Promise<void> {
  const feed = feedForChannel(channel);
  if (!feed) return;
  ...
  try {
    ...
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    manifest = (await res.json()) as SourceManifest;
  } catch (err) {
...
function runBuildScript(
  ...
  return new Promise((resolve, reject) => {
    ...
    const onData = (b: Buffer | string): void => {
      const s = String(b);
      try {
        logStream?.write(s);
      } catch {
...
let _building = false;
async function rebuildNow(): Promise<boolean> {
  ...
  try {
    ...
    setState({ status: 'error', error: String((err as { message?: unknown })?.message || err) });
    return false;
  } finally {
    _building = false;
  }
...
function runCheck(channel: UpdateChannel): void {
...
async function openDownload(): Promise<boolean> {
...
function checkNow(win?: BrowserWindow | null): void {
  if (win) _win = win;
...
function initUpdater(win: BrowserWindow): void {
  if (started) return;
  started = true;
  _win = win;
