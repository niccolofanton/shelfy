...
const JSONLD_TYPE_ALLOW = new Set([
  'organization', 'product', 'service', 'article', 'newsarticle', 'blogposting',
  'website', 'recipe', 'event', 'localbusiness', 'softwareapplication',
  'course', 'person', 'breadcrumblist',
]);
// Text-only fields lifted from an allowed JSON-LD block (never the raw object).
const JSONLD_TEXT_FIELDS = ['name', 'description', 'headline', 'brand', 'category', 'applicationCategory', 'articleSection'];
...
function stripTags(html) {
  ...
  while (i < n) {
    ...
    if (gt === -1) { out += ' '; break; } // unterminated '<…': drop the rest
...
function dropElements(html, tags) {
  ...
  for (const tag of tags) {
    out = out.replace(new RegExp(`<${tag}\\b[^>]{0,${ATTR_SCAN}}>[\\s\\S]*?<\\/${tag}>`, 'gi'), ' ');
...
const sanitizeUntrustedText = (s, opts) => sanitizeForPrompt(s, opts);
...
function extractLang(html) {
  const m = html.match(new RegExp(`<html\\b[^>]{0,${ATTR_SCAN}}\\blang\\s*=\\s*("([^"]*)"|'([^']*)'|([^\\s>]+))`, 'i'));
...
function extractHeadings(html) {
  ...
  for (const level of [1, 2, 3]) {
    ...
    while ((m = re.exec(html)) && out[`h${level}`].length < HEADING_CAP) {
      ...
      if (t && !seen.has(t.toLowerCase())) { seen.add(t.toLowerCase()); out[`h${level}`].push(t); }
    }
  }
  return out;
}
...
function extractJsonLd(html) {
  const out = [];
  const re = new RegExp(`<script\\b[^>]{0,${ATTR_SCAN}}type\\s*=\\s*("application/ld\\+json"|'application/ld\\+json'|application/ld\\+json)[^>]{0,${ATTR_SCAN}}>([\\s\\S]*?)<\\/script>`, 'gi');
  let m;
  while ((m = re.exec(html))) {
    let parsed;
    try { parsed = JSON.parse(m[2].trim()); } catch { continue; }
    ...
    for (const b of blocks) {
      ...
      for (const node of nodes) {
        if (node && typeof node === 'object' && jsonldTypeAllowed(node['@type'])) out.push(node);
      }
    }
  }
  return out;
...
function jsonldTypeAllowed(type) {
...
function jsonldTextFields(node) {
  const out = [];
  for (const f of JSONLD_TEXT_FIELDS) {
    let v = node[f];
    if (v && typeof v === 'object') v = v.name || v['@value'] || ''; // {name:'Acme'} brand shape
...
function extractMainText(html) {
  ...
  const chromeRe = /(^|[-_ ])(nav|menu|footer|header|cookie|consent|banner|sidebar|breadcrumb|share|social|newsletter|popup|modal)([-_ ]|$)/i;
  let cleaned = block.replace(new RegExp(`<(div|section|ul)\\b([^>]{0,${ATTR_SCAN}})>`, 'gi'), (full, tag, attrs) => {
    const cls = (attrs.match(/\b(?:class|id|role)\s*=\s*("([^"]*)"|'([^']*)')/i) || [])[2]
      || (attrs.match(/\b(?:class|id|role)\s*=\s*('([^']*)')/i) || [])[2] || '';
    return chromeRe.test(cls) || /aria-hidden\s*=\s*["']?true/i.test(attrs) ? `<${tag} data-chrome>` : full;
  });
...
function extractContent(html, url = '') {
  ...
  const metaDescription = metaByName(metas, 'description') || og.description || twitter.description || '';
...
function asPageContent(p) {
  if (!p || typeof p !== 'object') return null;
  if (p.content && typeof p.content === 'object') {
    return { ...p.content, title: p.title || p.content.title || '' };
  }
  return p;
}

// Dominant language among pages (first non-empty wins by frequency).
function dominantLang(langs) {
  const counts = {};
  for (const l of langs) { if (l) counts[l] = (counts[l] || 0) + 1; }
  let best = '';
  let n = 0;
  for (const [l, c] of Object.entries(counts)) { if (c > n) { n = c; best = l; } }
  return best;
}
 ...
 */
function aggregateSiteText(pages, opts = {}) {
  const maxChars = Math.max(1, Math.min(AGGREGATE_CAP, opts.maxChars || AGGREGATE_CAP));
  const list = (Array.isArray(pages) ? pages : []).map(asPageContent).filter(Boolean);

  const home = list[0] || {};
  const homeOg = home.og || {};
  ...
  const parts = [];
  const seenText = new Set(); // dedup identical normalized mainText across pages
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    for (const node of Array.isArray(p.jsonld) ? p.jsonld : []) {
      ...
      if (tlc.some((t) => t === 'organization' || t === 'product' || t === 'localbusiness') && typeof node.name === 'string') {
    ...
    if (mt && !seenText.has(key)) { seenText.add(key); pushPart(mt); }
  }
...
function resolveFfmpeg() {
  ...
  let userData = '';
  let resources = '';
  try { ({ app } = require('electron')); userData = app.getPath('userData'); } catch { /* non-electron ctx */ }
  try { resources = process.resourcesPath || ''; } catch { /* ignore */ }
  ...
  let staticPath = null;
  try { staticPath = require('ffmpeg-static'); } catch { /* optional */ }
  ...
  return firstExisting([staticPath, '/opt/homebrew/bin/ffmpeg', '/usr/bin/ffmpeg', '/usr/local/bin/ffmpeg']) || 'ffmpeg';
}
let app; // lazily filled by resolveFfmpeg() when running under Electron

// Reject a media path ffmpeg could misread as an option flag (leading '-').
function safeInputPath(p) {
...
function spawnAsync(bin, args, signal) {
 ...
 */
async function extractPalette(pageCtx = {}, signal) {
  // 1. CSS-first.
  try {
    if (pageCtx && typeof pageCtx.evaluate === 'function') {
      const res = await pageCtx.evaluate(PALETTE_EVAL_SRC);
...
function paletteFromSamples(samples) {
  const byHex = new Map(); // hex → { area, kinds:Set, r,g,b }
  let totalArea = 0;
  for (const s of samples) {
    ...
    totalArea += area;
    let e = byHex.get(hex);
    if (!e) { e = { area: 0, kinds: {}, r: c.r, g: c.g, b: c.b }; byHex.set(hex, e); }
    e.area += area;
  ...
  const rows = [...byHex.entries()].map(([hex, e]) => {
    ...
    let role = 'other';
    let best = 0;
    for (const [k, a] of Object.entries(e.kinds)) { if (a > best) { best = a; role = k; } }
  ...
  return top.map((r) => ({ hex: r.hex, role: r.role, weight: Math.round((r.weight / sum) * 100) / 100 }));
}

// Run ffmpeg palettegen → a tiny PNG of N swatches → read its colors.
async function paletteFromScreenshot(screenshotPath, signal) {
  ...
  try {
    ...
    const { code } = await spawnAsync(ffmpeg, [
      ...
      '-i', safe,
      '-vf', 'scale=320:-1:flags=area,palettegen=max_colors=8:stats_mode=full',
      '-y', out,
    ], signal);
    ...
    const withL = colors.map((c) => ({ ...c, L: relLuminance(c.r, c.g, c.b), S: rgbSaturation(c.r, c.g, c.b) }));
    ...
    return withL.slice(0, 8).map((c, i) => ({
      ...
      role: c === lightest ? 'background' : c === darkest ? 'text' : c === accent ? 'accent' : 'other',
...
function readPngPalette(file) {
  ...
  let raw;
  try { raw = require('zlib').inflateSync(Buffer.concat(idat)); } catch { return plte ? pltEntries(plte) : []; }
  ...
  for (let y = 0; y < height && rp < raw.length; y++) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      if (channels === 1) {
        const idx = cur[o];
        if (plte && idx * 3 + 2 < plte.length) { r = plte[idx * 3]; g = plte[idx * 3 + 1]; bb = plte[idx * 3 + 2]; }
        else { r = g = bb = cur[o]; }
      } else { r = cur[o]; g = cur[o + 1]; bb = cur[o + 2]; if (channels === 4) al = cur[o + 3]; }
      if (al === 0) continue;
      const hex = rgbToHex(r, g, bb);
      const e = seen.get(hex);
      if (e) e.n++; else seen.set(hex, { r, g, b: bb, n: 1 });
    }
    cur.copy(prev);
  }
  return [...seen.values()].sort((x, y) => y.n - x.n).slice(0, 8);
}
function pltEntries(plte) {
  const out = [];
  for (let i = 0; i + 2 < plte.length && out.length < 8; i += 3) out.push({ r: plte[i], g: plte[i + 1], b: plte[i + 2], n: 1 });
  return out;
}
...
const SYSTEM_FONTS = new Set([
  ...
  'roboto', 'system-ui', 'menlo', 'monaco', 'consolas',
]);

function fontProvider(family, ctx) {
  ...
  if (new RegExp('@font-face[\\s\\S]{0,400}' + fam.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i').test(html)) return 'self';
  return 'unknown';
}
 ...
 */
async function extractFonts(pageCtx = {}) {
  let info = {};
  try {
    if (pageCtx && typeof pageCtx.evaluate === 'function') {
      info = (await pageCtx.evaluate(FONTS_EVAL_SRC)) || {};
    }
  } catch { info = {}; }

  const ctx = { links: info.links || [], html: pageCtx.html || '' };
  // augment links with @font-face / google link parse from raw html (fallback)
  if (pageCtx.html) {
    const linkHrefs = (pageCtx.html.match(new RegExp(`<link\\b[^>]{0,${ATTR_SCAN}}href\\s*=\\s*["']([^"']+)["']`, 'gi')) || [])
      .map((s) => (s.match(/href\s*=\s*["']([^"']+)["']/i) || [])[1]).filter(Boolean);
    ctx.links = ctx.links.concat(linkHrefs);
  }

  const RANK = { heading: 3, body: 2, mono: 1, other: 0 };
  const byFamily = new Map();
  const add = (family, usage) => {
...
const TECH_RULES = [
  { name: 'next.js', test: (c) => /__NEXT_DATA__|\/_next\/static\//.test(c.html) || /next\.js/i.test(c.poweredBy) || !!c.runtime.next },
  { name: 'nuxt', test: (c) => /__NUXT__|\/_nuxt\//.test(c.html) || !!c.runtime.nuxt },
  { name: 'react', test: (c) => /data-reactroot|data-reactid|id=["']__next["']/.test(c.html) || !!c.runtime.react },
  { name: 'vue', test: (c) => /\bdata-v-[0-9a-f]{6,8}\b/.test(c.html) || !!c.runtime.vue },
  { name: 'svelte', test: (c) => /class=["'][^"']*\bsvelte-[0-9a-z]+/.test(c.html) || /\/_app\/immutable\//.test(c.html) },
  { name: 'gatsby', test: (c) => /id=["']___gatsby["']|\/page-data\//.test(c.html) || !!c.runtime.gatsby },
  { name: 'astro', test: (c) => /<astro-island|content=["']Astro["']/i.test(c.html) },
  { name: 'wordpress', test: (c) => /wp-content|wp-includes/.test(c.html) || /wordpress/i.test(c.generator) },
  { name: 'shopify', test: (c) => /cdn\.shopify\.com|Shopify\.theme/.test(c.html) || !!c.headers['x-shopify-stage'] || !!c.runtime.shopify },
  { name: 'webflow', test: (c) => /webflow/i.test(c.generator) || /\bclass=["'][^"']*\bw-(nav|container|button)\b/.test(c.html) || /assets\.website-files\.com|assets-global\.website-files\.com/.test(c.html) },
  { name: 'framer', test: (c) => /framer/i.test(c.generator) || /framerusercontent\.com/.test(c.html) },
  { name: 'wix', test: (c) => /static\.wixstatic\.com/.test(c.html) || Object.keys(c.headers).some((h) => h.startsWith('x-wix')) },
  { name: 'squarespace', test: (c) => /squarespace/i.test(c.generator) || /static1\.squarespace\.com/.test(c.html) },
  { name: 'gsap', test: (c) => /\bgsap(\.min)?\.js|cdn\.jsdelivr\.net[^"']*gsap/.test(c.html) || !!c.runtime.gsap },
  { name: 'three.js', test: (c) => /\bthree(\.min)?\.js\b/.test(c.html) || !!c.runtime.three },
  { name: 'lenis', test: (c) => /\blenis(\.min)?\.js\b|@studio-freight\/lenis/.test(c.html) || !!c.runtime.lenis },
...
function metaGeneratorOf(html) {
  const m = html.match(new RegExp(`<meta\\b[^>]{0,${ATTR_SCAN}}name\\s*=\\s*["']generator["'][^>]{0,${ATTR_SCAN}}>`, 'i'));
  if (!m) return '';
 ...
 */
async function detectTechStack(html, headers, pageCtx) {
  const safeHtml = typeof html === 'string' ? html : '';
  const hdr = lowerHeaders(headers);
  let runtime = {};
  try {
    if (pageCtx && typeof pageCtx.evaluate === 'function') {
      runtime = (await pageCtx.evaluate(TECH_RUNTIME_EVAL_SRC)) || {};
    }
  } catch { runtime = {}; }
  ...
  for (const rule of TECH_RULES) {
    ...
    if (hit && !seen.has(rule.name)) { seen.add(rule.name); found.push(rule.name); }
  }
  // Generic generator fallback (CMS/builder not in the table) → slug it.
  if (generator) {
    const slug = generator.toLowerCase().split(/[\s/]+/)[0].replace(/[^a-z0-9.-]/g, '');
    if (slug && slug.length > 1 && !seen.has(slug)) { seen.add(slug); found.push(slug); }
  }
  return found;
}
 ...
 */
async function buildWebMetadata(pageCtx = {}, signal) {
  const [palette, fonts, techStack] = await Promise.all([
    extractPalette(pageCtx, signal).catch(() => []),
    extractFonts(pageCtx).catch(() => []),
    detectTechStack(pageCtx.html, pageCtx.headers, pageCtx).catch(() => []),
  ]);
...
const EVIDENCE_CONFIDENCE = { 'self-link': 0.9, 'badge-img': 0.6, 'badge-script': 0.55, 'text-only': 0.25 };
const EVIDENCE_RANK = { 'self-link': 4, 'badge-img': 3, 'badge-script': 2, 'text-only': 1 };
...
function levelFromText(detector, text) {
  for (const [re, level] of detector.levels) { if (re.test(text)) return level; }
  return undefined;
}
function dateFromText(text) {
  const my = text.match(/\b(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*\.?\s+(\d{4})\b/i);
  if (my) {
    const mm = { jan: '01', feb: '02', mar: '03', apr: '04', may: '05', jun: '06', jul: '07', aug: '08', sep: '09', oct: '10', nov: '11', dec: '12' }[my[1].slice(0, 3).toLowerCase()];
    return `${my[2]}-${mm}`;
  }
 ...
 */
function detectAwards(pages, siteUrl, opts = {}) {
  const allowTextOnly = !!opts.allowTextOnly; // default OFF (high false-positive)
  const list = (Array.isArray(pages) ? pages : []).map((p) => (typeof p === 'string' ? { html: p } : p)).filter((p) => p && typeof p.html === 'string');
  ...
  for (const page of list) {
    ...
    for (const det of AWARD_DETECTORS) {
      ...
      const matchingSelf = platformEntryLinks.filter((a) => slug && pathOf(a.href).toLowerCase().includes(slug));
      ...
      if (selfLink) {
        ...
        consider({
          ...
          level: levelFromText(det, ctxText) || levelFromText(det, footerTextNear(footer, selfLink.href)),
      ...
      const badgeScript = scripts.find((s) => det.hosts.some((host) => s.includes(host)) && /badge|widget/i.test(s));
      if (badgeScript) {
        consider({ platform: det.platform, evidence: 'badge-script', confidence: EVIDENCE_CONFIDENCE['badge-script'] });
        continue;
      }
      ...
      if (allowTextOnly) {
        ...
        const named = new RegExp(`\\b${det.name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i').test(footerText);
        if (level || named) {
          consider({ platform: det.platform, level, evidence: 'text-only', confidence: EVIDENCE_CONFIDENCE['text-only'] });
        }
      }
    }
  }
...
function pathOf(u) {
  try { return new URL(u, 'https://x.invalid').pathname; } catch { return String(u || ''); }
}
function absolutize(u) { return String(u || ''); }

// Parse <a> tags into { href, text, title, ariaLabel }.
function matchAnchors(html) {
  const out = [];
...
function matchScriptSrcs(html) {
  ...
  let m;
  while ((m = re.exec(html))) { const s = attrVal(m[1], 'src'); if (s) out.push(s); }
  return out;
}
...
function footerTextNear(footer, needle) {
 ...
 */
function awardsToTagsEntities(awards) {
  const tags = new Set();
  const entities = new Set();
  const det = new Map(AWARD_DETECTORS.map((d) => [d.platform, d]));
  // platform → base tag slug
  const TAG_BASE = { awwwards: 'awwwards', cssda: 'css-design-awards', fwa: 'fwa', godly: 'godly', landbook: 'land-book', siteinspire: 'siteinspire' };
 ...
 */
function awardsQualityBoost(awards) {
  let score = 0;
  let strong = false;
  for (const a of Array.isArray(awards) ? awards : []) {
