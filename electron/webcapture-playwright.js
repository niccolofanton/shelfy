...
const { attachAdblock } = require('./adblock');
...
function dirHasHeadlessShell(dir) {
  try {
    return (
      fs.existsSync(dir) &&
      fs
...
async function getBrowser() {
  ...
  _browserPromise = (async () => {
    ...
    const launch = () => chromium.launch({ headless: true, args: LAUNCH_ARGS });
    let b;
    try {
      b = await launch();
    } catch (err) {
...
async function cookiesForPlaywright(partition) {
  try {
...
async function safeEval(page, code, { ms = 15_000, fallback } = {}) {
  let t;
  try {
...
async function waitForReady(page, { maxMs = READY_MAX_MS, signal } = {}) {
  const t0 = Date.now();
  for (;;) {
...
function frameSignature(png, signal) {
  return new Promise((resolve) => {
 ...
 * @param {string} url
 * @param {object} [opts]
 * @returns {Promise<object>} pageCtx
 */
async function capturePage(
  ...
  try {
    ...
    try {
      ...
      if (/Blocked redirect host/.test(e?.message || '')) throw e;
    ...
    const earlyLocked = await safeEval(page, JS_DOC_LOCKED, { fallback: false, ms: 5_000 });
    ...
    const canvasInfo = await safeEval(page, JS_DETECT_CANVAS, {
    ...
    const bigCanvas = await safeEval(page, JS_HAS_BIG_FIXED_CANVAS, { fallback: false, ms: 5_000 });
    ...
    if (jacked) {
      ...
      const html = await safeEval(page, 'document.documentElement.outerHTML', { fallback: '' });
      const title = (await safeEval(page, 'document.title', { fallback: '' })) || '';
    ...
    const vscroll = await safeEval(page, JS_NEUTRALIZE_VIRTUAL_SCROLL, { fallback: { libs: [] } });
    ...
    const fin = await safeEval(page, JS_FORCE_SCROLL_ANIM_END, {
    ...
    const docHeight = Number(
      await safeEval(page, 'Math.ceil(document.documentElement.scrollHeight)', {
        fallback: VIEWPORT_H,
      }),
    );
    ...
    try {
      try {
        ...
        mark(`pw-shot-failed(${err?.message || err})`);
        throw err;
      }
