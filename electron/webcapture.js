...
const { assertSafeUrl, isBlockedHostname } = require('./net-safety');
...
async function safeEval(wc, code, { ms = EVAL_TIMEOUT_MS, fallback } = {}) {
  try {
    if (wc.isDestroyed()) return fallback;
    return await withTimeout(wc.executeJavaScript(code, true), ms, 'eval');
  } catch {
    return fallback;
  }
}
...
function scorePath(pathname, homeLocale) {
...
function sectionKey(urlStr, homeLocale) {
  try {
...
function selectRepresentative(rawUrls, origin, finalUrl, maxPages) {
...
async function fetchText(url, { signal, accept, binary = false } = {}) {
...
async function fetchRobotsSitemaps(origin, ctx) {
  const sitemaps = [];
  try {
...
async function parseSitemap(sitemapUrl, ctx, depth = 0) {
 ...
 */
async function discoverPages(url, { maxPages = 6, signal } = {}) {
...
function screenshotPathForUrl(finalUrl, format = 'webp', stamp) {
  let host = 'site';
  try {
...
async function fetchImageToWebp(
  imageUrl,
  { pageUrl, stamp, format = 'webp', quality = 82, signal } = {},
) {
...
async function encodeImage(srcPng, outPath, format, quality, signal) {
...
function probeImageFlatness(file, signal) {
  let ffmpeg;
  try {
...
async function encodeFrames(framePngs, finalUrl, format, quality, { stamp, signal } = {}) {
  const chunks = [];
...
async function stitchSlices(slicePaths, outPath, format, quality, signal) {
  const ffmpeg = resolveFfmpeg();
...
async function probeImageSize(file, signal) {
  const ffmpeg = resolveFfmpeg();
...
function jsAutoScroll(maxHeightPx) {
  return `(async () => {
...
function waitForLoad(wc) {
  return new Promise((resolve, reject) => {
    ...
    const onStop = () => finish(resolve); // did-finish-load / did-stop-loading
    const onDomReady = () => {
      // DOM is parsed; give sync sub-resources a brief grace, then settle even if
      // the full load event never arrives.
      if (!graceTimer) graceTimer = setTimeout(() => finish(resolve), 2_000);
    };
    ...
    const timer = setTimeout(() => finish(resolve), NAV_TIMEOUT_MS); // settle on what loaded
    function cleanup() {
      clearTimeout(timer);
...
async function captureFullPage(wc, width, height) {
  let attached = false;
  try {
    try {
...
async function capturePage(
  ...
  const onWillDownload = (_e, item, webContents) => {
    if (webContents === wc) {
      try {
        item.cancel();
      } catch {}
  ...
  try {
    ...
    const vscroll = await safeEval(wc, JS_NEUTRALIZE_VIRTUAL_SCROLL, { fallback: { libs: [] } });
    ...
    const fin = await safeEval(wc, JS_FORCE_SCROLL_ANIM_END, { fallback: { st: 0, tweens: 0 } });
    ...
    const html = await safeEval(wc, 'document.documentElement.outerHTML', { fallback: '' });
    mark(`html(${(html || '').length})`);

    // 5) MEASURE + decide single-frame vs slice/stitch
    const measure = await safeEval(wc, JS_MEASURE, {
    ...
    const vis = await safeEval(wc, JS_DETECT_CANVAS, { fallback: { canvas: false } });
    if (vis && vis.canvas) {
      try {
        wc.invalidate();
      } catch {}
    ...
    try {
      ...
      mark(`cdp-failed(${err?.message || err})`);
    }
 ...
 */
async function captureMany(pages, opts = {}) {
  const { onPage, keepAlive = false, ...captureOpts } = opts;
  const list = (pages || []).map((p) => (typeof p === 'string' ? p : p && p.url)).filter(Boolean);
  const results = [];
  for (const u of list) {
