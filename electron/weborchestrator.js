...
const captureEngine = require('./capture-engine');
const enrich = require('./web-enrich');
const analyzer = require('./analyzer');
const { assertSafeUrl } = require('./net-safety');
...
const PHASE_WEIGHTS = { discovering: 0.1, capturing: 0.55, extracting: 0.25, analyzing: 0.1 };
const PHASE_BASE = { discovering: 0, capturing: 0.1, extracting: 0.65, analyzing: 0.9 };

// ─── State (mirror of analyzer.js / downloader.js) ────────────────────────────

const jobsMap = new Map(); // key → serializable job record
const urlCache = new Map(); // key → original url (needed for retry)
...
let onListRefresh = null; // ({ count, platform, refresh }) => void
...
function phaseProgress(phase, frac) {
...
function emitPhase(key, phase, frac, extra = {}) {
  patchJob(key, {
    status: phase,
    phase,
...
function pushEvent(key, kind, text, data) {
  const j = jobsMap.get(key);
  if (!j) return; // key cleared → no-op
  const prev = Array.isArray(j.events) ? j.events : [];
  const evt = { id: ++eventSeq, ts: Date.now(), phase: j.phase, kind, text };
...
function withTimeout(jobSignal, ms) {
...
async function captureWebReference(
  url,
  { signal, maxPages = DEFAULT_MAX_PAGES, onProgress, overwrite = false, singlePage = false } = {},
) {
  ...
  let webMetadata = null; // { palette, fonts, techStack } from the home
  ...
  const capturePageTask = async (discPage, i) => {
    ...
    try {
      ...
      live[i] = {
        ...
        chunks: Array.isArray(ctx.chunks) ? ctx.chunks : undefined,
        width: ctx.width,
        height: ctx.height,
        html: ctx.html,
        content,
      ...
      const reason =
        isAbortErr(err) || err?.name === 'TimeoutError' || /timeout/i.test(err?.message || '')
          ? 'timeout'
          : err?.message || 'errore';
      ...
      patchJob(key, { pages: live.filter(Boolean).map(toJobPage) });
    }
  };
  ...
  const captured = live.filter(Boolean);
  signal?.throwIfAborted?.();
  ...
  const applyOgFallback = async (cap) => {
    ...
    cap.chunks = [{ screenshotPath: ogPath, width: 0, height: 0 }];
  ...
  for (let i = 0; i < captured.length; i++) {
    ...
    try {
      ...
      cap.chunks = Array.isArray(ctx2.chunks) ? ctx2.chunks : undefined;
  ...
  let aggregate = null;
  let awards = [];
  let awardTagsEntities = { tags: [], entities: [] };
  {
  ...
  const webMeta = aggregate?.webMeta || {};
  const ref = {
    ...
    domain: domain || webMeta.siteName || null,
    title: webMeta.title || webMeta.siteName || domain || null,
    description: webMeta.description || captured[0]?.content?.metaDescription || null,
  ...
  let postId = ref.id;
  try {
    const res = db.upsertWebReference(ref, { overwriteAi: overwrite });
    ...
    throw new Error(`Salvataggio reference non riuscito: ${err?.message || err}`);
  }
  ...
  try {
    ...
    console.warn('[weborchestrator] analyzer enqueue failed:', err?.message);
  }
...
async function runJob(key) {
  ...
  try {
    ...
    if (isAbortErr(err)) {
      ...
      if (err?.noScreenshot && job.placeholderCreated) {
        try {
          db.deletePosts([job.postId]);
          onListRefresh?.({ count: 0, platform: 'web', refresh: true });
        } catch (e) {
          console.warn(`[weborchestrator] orphan placeholder cleanup failed:`, e?.message);
        }
      }
...
function enqueueWeb(
  url,
  { maxPages = DEFAULT_MAX_PAGES, overwrite = false, recovered = false, singlePage } = {},
) {
  ...
  // recovery) still wins.
  singlePage = singlePage === undefined ? !!existingPost?.webSinglePage : !!singlePage;
  try {
    db.createWebPlaceholder(url);
  } catch (err) {
    console.warn('[weborchestrator] placeholder creation failed:', err?.message);
    placeholderCreated = false;
  }
  ...
  setJob({
    ...
    finalUrl: url,
    domain,
    maxPages: singlePage ? 1 : clampMaxPages(maxPages),
    overwrite: !!overwrite,
...
function recover() {
  ...
  for (const job of rows) {
    ...
    try {
      ...
      keep.add(job.key);
      console.warn(`[weborchestrator] recover ${job.url} failed:`, e?.message);
    }
  }
...
function setProgressEmitter(fn) {
  onJobUpdate = typeof fn === 'function' ? fn : null;
}
function setListRefreshEmitter(fn) {
  onListRefresh = typeof fn === 'function' ? fn : null;
}

// Standalone discovery (preview), optional for the UI.
function discover(url, { maxPages = DEFAULT_MAX_PAGES } = {}) {
  assertSafeUrl(url);
  return webcapture.discoverPages(url, { maxPages: clampMaxPages(maxPages) });
}
