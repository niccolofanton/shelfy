...
// Detected host platform; 'unknown' when none of the known hostnames match.
type SelectPlatform = 'instagram' | 'pinterest' | 'twitter' | 'unknown';

// A normalized media slide ({ type, url }), as emitted by the DOM fallbacks.
interface SelectMedia {
  type: string;
  url: string;
}

// The post-like record resolved for a selection: either the rich captured item
// (window.__ssCapturedItems) or a minimal DOM-scraped fallback. A loose,
// structurally-typed superset of the fallback shapes built below.
...
// One post discovered in the grid: the card element we anchor the overlay to and
// its DOM-derived key (shortcode on IG, tweet/pin id elsewhere).
interface FoundPost {
  card: HTMLElement;
  key: string;
}

// boxOf result: whether `el` is a real, sized box (a valid overlay host) plus the
...
// The mutable overlay state. Custom (non-DOM) bookkeeping the script keeps for
// the lifetime of the page.
interface SelectState {
  enabled: boolean;
  platform: SelectPlatform;
  selected: Set<string>; // post keys (shortcode on IG, tweet id on TW)
  saved: Set<string>; // keys known to be in the local DB
  savedId: Map<string, string>; // key → DB id (to open the post modal on the host)
  queried: Set<string>; // ids already asked-about, to avoid re-querying the host
  decorated: Set<HTMLElement>; // card elements we've attached an overlay to
  hostByKey: Map<string, HTMLElement>; // key → card element (for DOM-fallback resolution)
  pos: Map<string, number>; // key → vertical offset within the grid (survives unmount)
  observer: MutationObserver | null;
  observerTarget: HTMLElement | null; // current MutationObserver target (grid root or body)
  rafPending: boolean;
  lastFound: number; // last reported post count (debug, avoids log spam)
  lastKey?: string | null; // shift-click anchor (the last-clicked checkbox key)
  rescanTimer?: ReturnType<typeof setInterval> | null; // periodic re-target/re-scan timer
}
...
// decorated card element. Declared here (not in a shared .d.ts) because they are
// private to this injected selection overlay. The capture-side window props
// (__socialSavedBridge / __ssCapturedItems) are declared by webview-injected.ts.
declare global {
  interface Window {
    __ssSelectInjected?: boolean;
    __ssSelect?: SelectApi;
    __ssDebug?: boolean;
  }
  interface HTMLElement {
    __ssDisp?: string;
    __ssWrap?: HTMLElement;
    __ssBox?: HTMLElement;
    __ssLabel?: HTMLElement;
    __ssKey?: string;
  }
}
...
(function () {
  ...
  function detectPlatform(): SelectPlatform {
  ...
  function captured(): { [key: string]: SelectItem } {
    return (window.__ssCapturedItems || {}) as { [key: string]: SelectItem };
  }
  ...
  function bestSrc(img: HTMLImageElement | null): string {
    if (!img) return '';
  ...
  function igFallback(card: HTMLElement, shortcode: string): SelectItem {
  ...
  function twFallback(card: HTMLElement, id: string): SelectItem {
  ...
  function pinFallback(card: HTMLElement, id: string): SelectItem {
    const url = bestSrc(card && card.querySelector('img'));
    return {
      id: id,
      platform: 'pinterest',
  ...
  function findPosts(): FoundPost[] {
    const out: FoundPost[] = [];
    if (state.platform === 'twitter') {
      const arts = document.querySelectorAll<HTMLElement>('article[data-testid="tweet"]');
  ...
  function boxOf(el: HTMLElement): BoxInfo {
  ...
  function postSel(): string {
    return state.platform === 'pinterest'
      ? 'a[href*="/pin/"]'
      : 'a[href*="/p/"], a[href*="/reel/"], a[href*="/tv/"]';
  }
  function pickCard(a: HTMLElement): HTMLElement {
  ...
  function recordPositions(posts: FoundPost[]): void {
  ...
  function decorate(card: HTMLElement, key: string): void {
  ...
  function update(card: HTMLElement, key: string): void {
  ...
  function selectRangeTo(key: string): void {
  ...
  function activate(key: string | null, withShift: boolean): void {
  ...
  function onCapture(e: Event): void {
    ...
    e.stopPropagation();
    if (e.stopImmediatePropagation) e.stopImmediatePropagation();
    if (e.type === 'click') activate(box.getAttribute('data-ss-key'), (e as MouseEvent).shiftKey);
  }
  ...
  function markSaved(pairs: unknown): void {
    ...
    let matchedDecor = 0;
    for (const p of pairs as Array<{ key?: unknown; id?: unknown } | null | undefined>) {
      if (!p || !p.key) continue;
