...
const { scoreClustering } = require('./score.cjs');
...
function setupScratch() {
  ...
  try {
    if (!fs.existsSync(linkModels) && fs.existsSync(realModels)) fs.symlinkSync(realModels, linkModels, 'dir');
...
function loadGold() {
  ...
  if (!fs.existsSync(file)) {
    if (goldArg) { console.error(`[cluster-eval] gold file not found: ${file}`); process.exit(2); }
    file = examplePath;
    placeholder = true;
    console.warn('\n[cluster-eval] ⚠  gold.json non trovato — uso gold.example.json (PLACEHOLDER).');
    console.warn('[cluster-eval]    I punteggi NON sono significativi finché non crei gold.json reale. Vedi README.md.\n');
  }
  ...
  return { goldLabels, nClusters: (parsed.clusters || []).length, nTags: goldLabels.size, placeholder, file };
}
...
async function predictedLabels(db) {
  const groups = await db.getTagCandidateGroups();
  const predLabels = new Map();
  groups.forEach((g, i) => {
...
async function main() {
  ...
  console.log('\n── Cluster eval (P10) ───────────────────────────────────────');
  console.log(`gold: ${path.basename(file)}${placeholder ? ' (PLACEHOLDER)' : ''} — ${nClusters} cluster, ${nTags} tag`);
  console.log(`produced: ${lastNGroups} gruppi | copertura gold: ${(lastCoverage * 100).toFixed(0)}% (${Math.round(lastCoverage * nTags)}/${nTags} tag valutati)`);
  console.log(`runs: ${RUNS}`);
  console.log('metric    median    mean     iqr      min      max');
  for (const k of ['ari', 'purity']) {
    const st = summary[k] || {};
    console.log(
  ...
  try { require('../../electron/embeddings.js').forceShutdown(); } catch { /* optional */ }
  process.exit(0);
}

main().catch((err) => {
  console.error('[cluster-eval] errore:', err && err.stack ? err.stack : err);
  process.exit(1);
});
