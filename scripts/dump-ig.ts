...
interface CollectReport {
  url: string;
  count: number;
  domOrderEqualsVisual: boolean;
  firstMismatchAt: number;
  domSeq: Array<string | null>;
  visSeq: Array<string | null>;
  tiles: Tile[];
}
...
async function snapshot(tag: string): Promise<CollectReport> {
  const wc = win!.webContents;
  const json = (await wc.executeJavaScript(COLLECT, true)) as string;
  fs.writeFileSync(path.join(OUT, `report-${tag}.json`), json);
  const html = (await wc.executeJavaScript(
    'document.querySelector("main") ? document.querySelector("main").outerHTML : document.body.outerHTML',
    true,
  )) as string;
  fs.writeFileSync(path.join(OUT, `dom-${tag}.html`), html);
  const rep = JSON.parse(json) as CollectReport;
  console.log(
    `[snap ${tag}] tiles=${rep.count} domOrder==visual? ${rep.domOrderEqualsVisual} mismatch@${rep.firstMismatchAt}`,
  );
  return rep;
}
...
async function scrollSteps(n: number, block: 'start' | 'end'): Promise<void> {
  for (let i = 0; i < n; i++) {
