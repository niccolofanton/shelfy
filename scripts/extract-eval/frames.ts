...
function grabFrame(file: string, t: number, out: string): boolean {
  const r = spawnSync(
    FFMPEG,
    [
      '-ss',
...
function scaleImage(file: string, out: string): boolean {
  const r = spawnSync(
    FFMPEG,
    [
      '-i',
...
for (const p of CASES.posts) {
  ...
  if (p.mediaType === 'video' && exists(p.videoPath)) {
    const dur = probeDuration(p.videoPath as string);
    for (let i = 0; i < N_FRAMES; i++) {
      const t = dur ? (dur * (i + 0.5)) / N_FRAMES : 0;
      const out = path.join(dir, `f${i}.jpg`);
      if (grabFrame(p.videoPath as string, t, out)) frames.push(path.basename(out));
      if (!dur) break;
    }
  } else if (p.mediaType === 'carousel' || p.mediaType === 'images' || (p.media || []).length > 1) {
    const slides = (p.media || [])
      .filter((m) => (m.type === 'image' || !m.type) && exists(m.localPath))
      .slice(0, CAROUSEL_MAX);
    slides.forEach((m, i) => {
      const out = path.join(dir, `f${i}.jpg`);
      if (scaleImage(m.localPath as string, out)) frames.push(path.basename(out));
    });
  } else {
    const single = [p.imagePath, p.thumbnailPath, ...(p.media || []).map((m) => m.localPath)].find(
      exists,
    );
    if (single) {
  ...
  console.log(
    `${p.mediaType.padEnd(9)} ${String(p.id).slice(0, 24).padEnd(24)} → ${frames.length} frame${frames.length === 1 ? '' : 's'}`,
  );
}
