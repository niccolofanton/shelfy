...
const GT = fs.existsSync(GT_PATH) ? JSON.parse(fs.readFileSync(GT_PATH, 'utf8')) : null;
const { scoreCase } = require('./score.cjs');
const { summarize } = require('../lib/agg-stats.cjs');
...
let results = new Map(); // postId -> job (rimpiazzata a ogni run con --runs)
const t0 = Date.now();
...
(async () => {
  ...
  const runComposites = new Map(rows.map((r) => [r.id, [r.s.composite]]));
