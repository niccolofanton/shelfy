...
const compact = (t: unknown): string => norm(t).replace(/\s+/g, '');
const stem = (t: unknown): string =>
  norm(t)
    .split(' ')
    .map((w) => (w.length > 4 ? w.replace(/(s|i|e)$/u, '') : w))
    .join(' ');

function synGroup(t: unknown): string[] {
  const n = norm(t);
...
function matchTag(a: unknown, b: unknown): boolean {
  const na = norm(a),
    nb = norm(b);
  ...
  if (
    ...
    (short.length > 1 || short[0].length >= 4)
  )
    return true;
  // synonyms
  const ga = synGroup(a);
...
const matchesAny = (tag: string, list?: string[]): boolean =>
  (list || []).some((g) => matchTag(tag, g));
...
function matchConcept(term: string, pool: string[]): boolean {
...
function matchForbidden(modelTag: string, forbiddenTag: string): boolean {
  const na = norm(modelTag),
    nb = norm(forbiddenTag);
  if (!na || !nb) return false;
  if (
    na === nb ||
    compact(modelTag) === compact(forbiddenTag) ||
    stem(modelTag) === stem(forbiddenTag)
  )
    return true;
  if (synGroup(modelTag).includes(nb) || synGroup(forbiddenTag).includes(na)) return true;
  const tModel = na.split(' '),
    tF = nb.split(' ');
  if (tF.every((w) => tModel.includes(w))) return true; // model tag ⊇ forbidden term
  return false;
}
const matchesAnyForbidden = (tag: string, list?: string[]): boolean =>
  (list || []).some((g) => matchForbidden(tag, g));

function scoreCase(gt: GroundTruth, out: ModelOutput): CaseScore {
  ...
  const subjInText = subj.length
    ? contentTokens(subj.join(' ')).some(
        (w) => norm(desc).includes(w) || kws.some((k) => norm(k).includes(w)),
      )
    : false;
  ...
  if (ideal.length) {
    ...
    const hit = ideal.filter((ik) => {
      const toks = contentTokens(ik);
      return toks.length && toks.some((w) => haystack.includes(w));
    });
  ...
  const composite = Math.max(
    ...
    Math.min(
      1,
      0.45 * recall + 0.25 * coverage + 0.15 * kwScore + 0.15 * subjectHit - 0.5 * forbiddenRate,
    ),
  );
  ...
  return {
    ...
    matchedMust,
    missedMust: mustHave.filter((g) => !matchedMust.includes(g)),
    forbiddenHits: forbiddenHits,
    tags,
  };
}

export { norm, matchTag, matchesAny, scoreCase, SYNONYMS };
export type { GroundTruth, ModelOutput, CaseScore };
