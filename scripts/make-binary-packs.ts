...
const exe = (b: string): string => (isWin ? `${b}.exe` : b);
...
function ffmpegSource(): string | null {
  if (isWin) return path.join(ROOT, 'bin', 'ffmpeg.exe');
  try {
    return require('ffmpeg-static') as typeof import('ffmpeg-static').default;
  } catch {
...
for (const [rel, src] of Object.entries(sources)) {
  if (!existsSync(src as string)) die(`missing source for "${rel}": ${src}`);
}
...
for (const [rel, src] of Object.entries(sources)) {
  ...
  cpSync(src as string, dest, {
    recursive: statSync(src as string).isDirectory(),
