...
function dedupe(ranked: string[]): string[] {
...
function precisionAtK(ranked: string[], gold: Set<string>, k: number): number | null {
...
function recallAtK(ranked: string[], gold: Set<string>, k: number): number | null {
...
function mrr(ranked: string[], gold: Set<string>): number | null {
  if (!gold || !gold.size) return null;
...
function ndcgAtK(ranked: string[], gold: Set<string>, k: number): number | null {
  if (!gold || !gold.size) return null;
  const r = dedupe(ranked).slice(0, k);
  let dcg = 0;
  for (let i = 0; i < r.length; i++) {
...
function orderMetrics(ranked: string[], gold: Set<string>): Record<string, number | null> {
  return {
...
export { precisionAtK, recallAtK, mrr, ndcgAtK, orderMetrics, dedupe };
