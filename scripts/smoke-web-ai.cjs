...
async function main() {
  ...
  const ref = {
    url: TARGET_URL, finalUrl: TARGET_URL, domain: new URL(TARGET_URL).hostname,
    title: content.title, description: content.metaDescription, lang: content.lang,
    pages: [{ url: TARGET_URL, pageType: 'home', screenshotPath: SURROGATE, contentText: agg.contentText, title: content.title }],
    palette: [], fonts: [], techStack: tech, awards,
  ...
  log('domain:', post.webDomain, '| image_path:', post.imagePath, '| esiste:', !!(post.imagePath && fs.existsSync(post.imagePath)));
  log('media slide:', (post.media || []).length, '| webTech:', JSON.stringify(post.webTech), '| webAwards:', JSON.stringify(post.webAwards));
...
const hardStop = setTimeout(() => { console.error('[smoke-ai] HARD TIMEOUT'); try { app.exit(2); } catch {} }, 320000);
