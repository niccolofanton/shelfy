...
import * as orch from '../electron/weborchestrator';
...
// captureWebReference resolves void; the original reads .id/.postId defensively
// (the function does not actually return them, so this stays undefined and the
// getPosts fallback below takes over). Typed precisely so the defensive read
// compiles without altering runtime.
type CaptureResult = { id?: string; postId?: string } | undefined;
...
async function main(): Promise<void> {
  ...
  try {
    ...
    log('CAPTURE ERROR:', (e && (e as Error).stack) || e);
  }
  ...
  if (!id) {
    try {
      ...
      const arr = ((r && r.posts) || r || []) as unknown as Shelfy.Post[];
      if (arr[0]) id = arr[0].id;
    } catch (e) {
      log('fallback getPosts err:', (e as Error).message);
    }
  }
  log('post id:', id);
