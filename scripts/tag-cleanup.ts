...
function normalizeTagRows(tags: readonly unknown[]): Array<{ norm: string; form: string }> {
  const seen = new Set<string>();
  const out: Array<{ norm: string; form: string }> = [];
  for (const t of tags) {
...
// if unchanged.
function rewrite(rawTags: unknown, map: Map<string, string>, drop: Set<string>): unknown[] | null {
...
const drop = new Set<string>(DELETE.map((d) => d.toLowerCase()));
...
if (DELETE_PREFIXES && DELETE_PREFIXES.length) {
  const allNorms = db
    .prepare('SELECT DISTINCT tag_norm FROM post_tags')
    .all()
    .map((r) => (r as { tag_norm: string }).tag_norm);
...
const rows = db.prepare('SELECT id, ai_tags FROM posts WHERE ai_tags IS NOT NULL').all() as Array<{
  id: string;
  ai_tags: string | null;
}>;
...
const countOf = (norm: string): number =>
  (
    db.prepare('SELECT COUNT(DISTINCT post_id) c FROM post_tags WHERE tag_norm = ?').get(norm) as {
      c: number;
    }
  ).c;
