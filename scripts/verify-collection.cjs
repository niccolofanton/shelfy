...
// ---- accumulators -----------------------------------------------------------
const domCodes = new Set(); // shortcodes seen as visible tiles
const netCodes = new Set(); // shortcodes seen in intercepted JSON feed responses
let moreAvailable = null; // last `more_available` flag from REST feed responses
...
function harvest(obj, depth = 0) {
  ...
  if (typeof obj.more_available === 'boolean') moreAvailable = obj.more_available;
  ...
  if (typeof code === 'string' && /^[A-Za-z0-9_-]{6,15}$/.test(code) && (obj.pk || obj.id)) {
    netCodes.add(code);
  }
  for (const k of Object.keys(obj)) harvest(obj[k], depth + 1);
}

function attachNetwork(wc) {
...
async function collectDom(wc) {
  try {
    const codes = await wc.executeJavaScript(COLLECT_DOM, true);
    codes.forEach((c) => domCodes.add(c));
  } catch (_) {}
}

async function scrollOnce(wc) {
...
async function run() {
  ...
  const dbCodes = new Set(dbInfo.posts.map((p) => p.shortcode).filter(Boolean));
