...
function extractFrames(file, outDir) {
...
async function waitHealth(port, ms = 120000) {
  const t0 = Date.now();
...
async function main() {
  ...
  const srvError = new Promise((_, rej) => {
  ...
  try {
    ...
    try {
      ...
      console.log('⚠ output non è JSON valido:', e.message);
    }
  } finally {
