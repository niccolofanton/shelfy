...
interface InspectResult {
  ...
  innerH: number;
  maxScrollY: number;
  canvases: Array<{ w: number; h: number; pos: string; z: string }>;
  scrollers: Array<{ tag: string; id: string; cls: string; sh: number; ch: number }>;
  htmlClass: string;
  bodyClass: string;
}
...
async function run(): Promise<void> {
  ...
  const after = (await pg.evaluate(INSPECT)) as InspectResult;
  console.log(
    JSON.stringify(
      {
