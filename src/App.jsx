...
const VIEW_IDS = ['gallery', 'downloads', 'aitags', 'aiqueue', 'aiweb', 'aisearch', 'settings'];
...
const AI_VIEW_IDS = ['aitags', 'aiqueue', 'aiweb', 'aisearch'];
...
const WEB_ACTIVE_STATUS = ['pending', 'discovering', 'capturing', 'extracting', 'analyzing'];
...
function AppInner() {
  ...
  const [showDisclaimer, setShowDisclaimer] = useState(shouldShowDisclaimerGate);
  ...
  const [mountedViews, setMountedViews] = useState(() => new Set(['gallery']));
  const [browserTab, setBrowserTab] = useState('instagram');
  const [collectionModal, setCollectionModal] = useState(null); // null | { initial?: Collection }
  ...
  const [newPostsAlert, setNewPostsAlert] = useState({ instagram: 0, twitter: 0, pinterest: 0 });
  // Live sync state per source, reported up by Browser so the sidebar can show an indicator.
  const [browserSyncing, setBrowserSyncing] = useState({
    instagram: false,
    twitter: false,
    pinterest: false,
  });
  const [activeSource, setActiveSource] = useState({ type: 'platform', value: 'all' });
  const [gallerySourceNonce, setGallerySourceNonce] = useState(0); // bumped on each Sources click
  const [devBarVisible, setDevBarVisible] = useState(false);
  const devBarMounted = useRef(false);
  ...
  const browserTabRef = useRef(browserTab);
  const browserSyncingRef = useRef(browserSyncing);
  ...
  const sourceSyncApiRef = useRef(null);
  const registerSourceSyncApi = useCallback((api) => {
    sourceSyncApiRef.current = api;
  }, []);
  const [sourceSyncJobs, setSourceSyncJobs] = useState({});
  const sourceSyncJobsRef = useRef(sourceSyncJobs);
  ...
  const handleSyncSource = useCallback((target) => {
  ...
  const [aiTagsInitial, setAiTagsInitial] = useState({ tag: null, nonce: 0 });
  const openAiPost = useCallback(async (postId) => {
    if (!postId) return;
    try {
  ...
  const goReanalyzeWeb = useCallback(async (p) => {
  ...
  const openEditCollection = useCallback((c) => setCollectionModal({ initial: c }), []);
  const handleBrowserSaved = useCallback(
    ({ count, platform }) => setLastSave({ count, platform, ts: Date.now() }),
    [],
  );
  ...
  const handleSaveCollection = async ({ name, color }) => {
  ...
  const handleDeleteCollection = async (id, opts = {}) => {
  ...
  useEffect(() => {
    ...
    const unsub = window.electronAPI.onNewPosts((data) => {
      bumpStats();
  ...
  const galleryPlatform = activeSource.type === 'platform' ? activeSource.value : 'all';
  const galleryCollectionId = activeSource.type === 'collection' ? activeSource.value : null;
        ...
        <main className="flex-1 overflow-hidden relative">
          ...
          <div style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
            <BrowserMemo
              ...
              onSourceSyncJobs={setSourceSyncJobs}
            />
          </div>
            ...
            {aiGate && (
              <div
                ...
                style={{
                  ...
                  zIndex: 5,
                  background: '#0f0f0f',
                  visibility: AI_VIEW_IDS.includes(view) ? 'visible' : 'hidden',
                  opacity: AI_VIEW_IDS.includes(view) ? 1 : 0,
                  transition: 'opacity var(--dur-3) var(--ease-out)',
                }}
              >
                <AiOnboarding
        ...
        {aiModalPost && (
          <PostModal
            ...
            onApplyAiFilter={(patch) => {
              setAiModalPost(null);
              if (patch?.tag) {
                setAiTagsInitial((s) => ({ tag: patch.tag, nonce: s.nonce + 1 }));
                setView('aitags');
              }
            }}
