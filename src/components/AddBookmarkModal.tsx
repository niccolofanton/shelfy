...
// The four bookmark kinds classifyFile resolves a File into. Derived from the
// classifier's own return type so the modal stays in lockstep with the lib.
type FileKind = ReturnType<typeof classifyFile>;

// One selected file in the modal's working set: the File plus a cheap inline
// preview (object URL for images, null otherwise) and its classified kind.
interface FileItem {
  id: number;
  file: File;
  kind: FileKind;
...
type AddBookmarkStatus = 'idle' | 'working' | 'error';
...
function KindIcon({ kind, size = 16 }: KindIconProps) {
...
function FileRow({ item, onRemove, removeLabel }: FileRowProps) {
  return (
 ...
 */
export default function AddBookmarkModal({ onClose, onAdded }: AddBookmarkModalProps) {
  const t = useT('addBookmark');
  const tc = useT('common');
  const [status, setStatus] = useState<AddBookmarkStatus>('idle');
  ...
  useEffect(() => {
    return () => itemsRef.current.forEach((it) => it.thumbUrl && URL.revokeObjectURL(it.thumbUrl));
  }, []);
  ...
  async function handleSubmit(): Promise<void> {
    ...
    try {
      ...
      const finalTags = tagDraft.trim()
        ...
        : tags;
      const files: Awaited<ReturnType<typeof prepareFile>>[] = [];
