...
function Favicon({ domain, size = 11 }) {
...
function WebDomainBadge({ domain }) {
...
function MediaTypeIcon({ mediaType, mediaCount = 1 }) {
...
function OfflineIcon({ isDownloaded, t }) {
  if (isDownloaded) {
...
function WebFallback({ post, t }) {
  const title = post.webMeta?.title || post.webMeta?.siteName || null;
  return (
    <div
...
function ManualFallback({ post, t }) {
  return (
    <div
...
function SocialFallback({ post, t }) {
  return (
    <div
...
function buildSlideshowImages(post) {
...
function PostCard({ post, onOpen, selectable = false, selected = false, onQuickSelect }) {
  ...
  const firstTextLine = useMemo(
    () => (hasText ? post.text.split('\n').map((l) => l.trim()).find(Boolean) : null),
    [hasText, post.text],
  );
  ...
  const handleImageRef = useCallback((el) => {
  ...
  function handleQuickSelectKeyDown(e) {
    if (e.key === 'Enter' || e.key === ' ' || e.key === 'Spacebar') {
      e.preventDefault();
      e.stopPropagation();
      onQuickSelect(post, e);
