...
import MetaColumn from './postmodal/MetaColumn';
...
export default function PostModal({
  ...
  const handleAiEditingChange = useCallback((editing) => {
  ...
  const [assignedIds, setAssignedIds] = useState(() => new Set(post.collectionIds || []));
  ...
  const goSlidePrev = () => setSlide((s) => (s > 0 ? s - 1 : s));
  const goSlideNext = () => setSlide((s) => (s < slideCount - 1 ? s + 1 : s));
  ...
  async function assignToCollection(cid) {
    const pid = post.id;
  ...
  async function handleCreateAndAssign({ name, color }) {
    try {
          ...
          images={imageSlides.map(({ s }, i) => ({
            ...
            chunks: isWeb
              ? (() => {
                  ...
                  return pg
                    ? pg.chunks.map((c) => assetUrl(c.screenshotPath)).filter(Boolean)
                    : undefined;
                })()
              : undefined,
            label: isWeb
