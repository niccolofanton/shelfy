...
function Sidebar({
  ...
  const [expandedGroups, setExpandedGroups] = useState(() =>
    loadPersisted(LS_GROUPS_KEY, { browser: true, bookmarks: true, ai: true }),
  );
  ...
  const [expandedPlatforms, setExpandedPlatforms] = useState(() =>
    loadPersisted(LS_PLATFORMS_KEY, {}),
  );
  ...
  const isNewRow = (id) => animatedIds == null || !animatedIds.has(id);
  ...
  const renderCollectionRow = (c, i, { nested = false, isLast = false } = {}) => {
                ...
                {open && (
                  <div className="flex flex-col gap-0.5 mb-0.5">
                    {BROWSER_TABS.map((t, i) => {
                      const subActive = currentView === 'browser' && browserTab === t.id;
                      const count = newPostsAlert?.[t.id] || 0;
                      const syncing = !!browserSyncing?.[t.id];
                      ...
                      return (
                        ...
                        >
                          ...
                          {syncing && (
                            <RefreshCw
                              size={12}
                              data-testid={`browser-tab-${t.id}-syncing`}
                              className="shrink-0 text-amber-400 u-spin"
                            />
                          )}
                          {count > 0 && (
                            <span
                              data-testid={`browser-tab-${t.id}-badge`}
