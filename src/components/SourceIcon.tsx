...
export const PLATFORM_COLORS: Record<Shelfy.Platform, string> = {
  instagram: '#e1306c',
  pinterest: '#e60023',
  twitter: '#1da1f2',
  web: '#7B5CFF',
...
function InstagramGlyph({ size, className, ...props }: GlyphProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
...
function XGlyph({ size, className, ...props }: GlyphProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
...
interface SourceIconProps extends React.SVGProps<SVGSVGElement> {
  platform: Shelfy.Platform | null | undefined;
  size?: number;
  className?: string;
}
...
export default function SourceIcon({
  ...
  className = '',
  ...props
}: SourceIconProps): React.ReactElement | null {
