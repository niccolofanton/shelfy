...
function UserLayer({ note, manualTags, onApplyAiFilter, onChangeNote, onChangeManualTags }) {
  const t = useT('postModal');
...
export default function AiPanel({ post, onApplyAiFilter, onPostUpdated, onEditingChange }) {
  const t = useT('postModal');
  const tc = useT('common');
  const {
    jobFor,
  ...
  const handleChangeNote = async (text) => {
  ...
  const handleChangeManualTags = async (arr) => {
  ...
  const [draftSaveReason, setDraftSaveReason] = useState('');
  const [tagInput, setTagInput] = useState('');
  // Tags currently playing their exit (u-pop-out) animation before removal.
  const [removingTags, setRemovingTags] = useState(() => new Set());
  const [saving, setSaving] = useState(false);
  // Two-step confirmation for deleting the AI description.
  const [confirmDeleteDesc, setConfirmDeleteDesc] = useState(false);
  ...
  const [confirmDeleteTags, setConfirmDeleteTags] = useState(false);
  ...
  const removeTag = (t) => setRemovingTags((prev) => new Set(prev).add(t));
  const finalizeRemoveTag = (t) => {
    ...
    return (
      <Section>
        <div className="flex items-center gap-2 text-xs text-[#bdbdbd]">
          ...
          {t('downloadingModel', { percent: Math.round((modelProgress.progress || 0) * 100) })}
        </div>
      ...
      <Section>
        <div className="flex items-center justify-between gap-2">
          <span className="flex items-center gap-2 text-xs text-[#bdbdbd]">
            <Loader2 size={13} className="animate-spin text-violet-400" />
            {ANALYZE_STATUS_KEY[status] ? t(ANALYZE_STATUS_KEY[status]) : t('processing')}
          </span>
          <button
            onClick={() => cancelJob(job.key)}
  ...
  return (
    <Section>
      ...
      <button
        ...
        onClick={status === 'error' ? () => retryJob(job.key) : handleAnalyze}
