...
function fontEntries(fonts: unknown): Shelfy.WebFont[] {
  if (!Array.isArray(fonts)) return [];
  return (fonts as unknown[])
    .map((f): Shelfy.WebFont | null => {
      if (typeof f === 'string') return { family: f };
      if (f && typeof f === 'object') return f as Shelfy.WebFont;
      return null;
    })
    .filter(
      (f): f is Shelfy.WebFont =>
        f != null && typeof f.family === 'string' && Boolean(f.family.trim()),
    );
}
// Tech is an array of slug strings.
function techList(tech: unknown): string[] {
  if (!Array.isArray(tech)) return [];
  return (tech as unknown[]).filter((t): t is string => typeof t === 'string' && Boolean(t.trim()));
}
// Awards are objects { platform, level?, date?, profileUrl?, evidence, confidence }.
// `evidence` isn't part of the persisted Shelfy.WebAward shape but the loose
// capture data carries it, so it's surfaced (as the tooltip) via this extension.
type WebAwardEntry = Shelfy.WebAward & { evidence?: string };
function awardList(awards: unknown): WebAwardEntry[] {
  if (!Array.isArray(awards)) return [];
  return (awards as unknown[]).filter(
    (a): a is WebAwardEntry =>
      a != null &&
      typeof a === 'object' &&
      Boolean((a as WebAwardEntry).platform || (a as WebAwardEntry).level),
  );
}
...
function PaletteSwatch({ hex }: PaletteSwatchProps) {
...
export default function WebMetaPanel({
  post,
  onOpenInWebsites,
  onReanalyzeWeb,
}: WebMetaPanelProps) {
