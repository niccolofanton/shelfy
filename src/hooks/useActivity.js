...
const PLATFORM_LABEL = { instagram: 'Instagram', twitter: 'X', pinterest: 'Pinterest' };
...
function webPhaseLabel(phase, t) {
  return WEB_PHASES.has(phase) ? t('phase_' + phase) : undefined;
}
const webKey = (j) => (j?.postId ? `web:${j.postId}` : j?.key);
function hostFrom(j, t) {
  if (j?.domain) return j.domain;
  try {
    return new URL(j?.finalUrl || j?.url).hostname.replace(/^www\./, '');
  } catch {
...
// }
export function buildActivities(sources = {}, lang = 'it') {
  const t = (k, vars) => translate(lang, 'activity.' + k, vars);
  const live = [];
  ...
  for (const platform of Object.keys(syncJobs)) {
    ...
    if (j.status === 'navigating' || j.status === 'syncing') {
      ...
      if (j.stepCount > 1) parts.push(`${(j.stepIndex ?? 0) + 1}/${j.stepCount}`);
...
function useActivityCore(sources) {
  const { lang } = useLang();
  const t = useCallback((k, vars) => translate(lang, 'activity.' + k, vars), [lang]);
  const [updateState, setUpdateState] = useState(null);
  const [binariesProgress, setBinariesProgress] = useState(null);
  const [sttProgress, setSttProgress] = useState(null);
  const [log, setLog] = useState([]); // storico { id, kind, title, subtitle, status, ts, read }
  ...
  const applyUpdateState = useCallback((s) => {
    setUpdateState(s);
  ...
  const pushLog = useCallback((entry) => {
    ...
    setLog((prev) =>
      [{ read: false, ...entry, id, ts }, ...prev.filter((e) => e.id !== id)].slice(0, LOG_CAP),
    );
  }, []);
  const dismiss = useCallback((id) => setLog((prev) => prev.filter((e) => e.id !== id)), []);
  const clearAll = useCallback(() => setLog([]), []);
  const markAllRead = useCallback(
    () => setLog((prev) => prev.map((e) => (e.read ? e : { ...e, read: true }))),
    [],
  );
  ...
  useEffect(() => {
    ...
    api
      .getUpdateState?.()
      .then((s) => {
        if (s) applyUpdateState(s);
      })
      .catch(() => {});
    const offUpd = api.onUpdaterState?.((s) => applyUpdateState(s));
    const offBin = api.onBinariesProgress?.((p) => setBinariesProgress(p));
    const offStt = api.onSttModelProgress?.((p) => setSttProgress(p));
    return () => {
      offUpd?.();
      offBin?.();
      offStt?.();
...
export function ActivityProvider({ children, ...sources }) {
