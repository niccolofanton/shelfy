...
// Minimal imperative surface of the Electron <webview> guest element the sync
// machinery touches. Mirrors ElectronWebview in views/Browser.tsx so the shared
// per-tab ref containers are assignable across the boundary.
...
// Shared per-tab ref containers (created once with useRef(...).current). Each tab
// owns a mutable box so the value can change without re-rendering.
...
// Per-tab counters mirrored synchronously by ingestBatch.
interface SyncCounts {
  scanned: number;
  fresh: number;
}

// The pre-sync capture buffer (per tab): items stashed before sync starts, keyed
// by the listing URL they belong to.
interface PendingBuffer {
  url: string;
  items: SanitizedItem[];
}

export interface UseBrowserSyncOptions {
  activeTab: BrowserTab;
  webviewRefs: WebviewRefs;
  injectedScriptRef: React.MutableRefObject<string>;
  onSyncingChange?: (syncing: PerTabState<boolean>) => void;
  onCollectionsChanged?: () => void;
}

export interface UseBrowserSync {
  syncing: PerTabState<boolean>;
  syncingRef: React.MutableRefObject<PerTabState<boolean>>;
  interceptedCount: PerTabState<number>;
  syncScanned: PerTabState<number>;
  syncNew: PerTabState<number>;
  syncSkipped: PerTabState<number>;
  syncElapsed: PerTabState<number>;
  libraryTotal: number | null;
  setLibraryTotal: React.Dispatch<React.SetStateAction<number | null>>;
  syncTimerRefs: PerTabRefMap<ReturnType<typeof setInterval> | null>;
  syncCollectionRefs: PerTabRefMap<number | null>;
  syncScriptPromiseRefs: PerTabRefMap<Promise<unknown> | null>;
  syncCountsRefs: PerTabRefMap<SyncCounts>;
  pendingRefs: PerTabRefMap<PendingBuffer>;
  ...
  startSync: (tab?: BrowserTab, collectionId?: number | null) => boolean;
...
export default function useBrowserSync({
...
}: UseBrowserSyncOptions): UseBrowserSync {
  // All sync-related counters are per-tab so Instagram and Twitter can sync concurrently.
  const [interceptedCount, setInterceptedCount] = useState<PerTabState<number>>({ ...ZERO });
  const [syncing, setSyncing] = useState<PerTabState<boolean>>({ ...FALSE });
  const [syncScanned, setSyncScanned] = useState<PerTabState<number>>({ ...ZERO });
  const [syncNew, setSyncNew] = useState<PerTabState<number>>({ ...ZERO });
  const [syncSkipped, setSyncSkipped] = useState<PerTabState<number>>({ ...ZERO });
  const [syncElapsed, setSyncElapsed] = useState<PerTabState<number>>({ ...ZERO });
  ...
  const syncTimerRefs = useRef<PerTabRefMap<ReturnType<typeof setInterval> | null>>({
  ...
  const syncGenRefs = useRef<PerTabRefMap<number>>({
    instagram: { current: 0 },
    twitter: { current: 0 },
    pinterest: { current: 0 },
  }).current;
  ...
  const syncScriptPromiseRefs = useRef<PerTabRefMap<Promise<unknown> | null>>({
    instagram: { current: null },
    twitter: { current: null },
    pinterest: { current: null },
  }).current;
  ...
  const syncCountsRefs = useRef<PerTabRefMap<SyncCounts>>({
  ...
  const syncCollectionRefs = useRef<PerTabRefMap<number | null>>({
    instagram: { current: null },
    twitter: { current: null },
    pinterest: { current: null },
  }).current;
  ...
  const pendingRefs = useRef<PerTabRefMap<PendingBuffer>>({
    instagram: { current: { url: '', items: [] } },
    twitter: { current: { url: '', items: [] } },
    pinterest: { current: { url: '', items: [] } },
  }).current;
  const syncingRef = useRef<PerTabState<boolean>>({ ...FALSE });
  ...
  const onCollectionsChangedRef = useRef<(() => void) | undefined>(onCollectionsChanged);
  useEffect(() => {
  ...
  useEffect(() => {
    return () => {
      for (const tab of TABS) {
        const ref = syncTimerRefs[tab.id as BrowserTab];
  ...
  const finishSync = (tab: BrowserTab, gen: number | null = null): void => {
  ...
  const startSync = (tab: BrowserTab = activeTab, collectionId: number | null = null): boolean => {
    const wv = webviewRefs[tab].current;
    if (!wv) return false;
  ...
  const stopSync = (tab: BrowserTab = activeTab): void => finishSync(tab);
