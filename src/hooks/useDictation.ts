...
// Narrow an unknown bridge value to an SttModelStatus.
function asSttStatus(v: unknown): SttModelStatus | null {
...
export interface UseDictationResult {
  ...
  subscribeAudioLevel: (cb: (lvl: number) => void) => () => void;
  error: string | null;
  modelStatus: SttModelStatus | null;
  modelProgress: SttModelProgress | null;
...
export function useDictation({
  ...
  const [modelStatus, setModelStatus] = useState<SttModelStatus | null>(null); // { modelReady, binaryReady, ready, downloading }
  const [modelProgress, setModelProgress] = useState<SttModelProgress | null>(null); // { progress, label } during download

  const recRef = useRef<DictationRecorder | null>(null);
  ...
  const audioSubsRef = useRef<Set<(lvl: number) => void>>(new Set());
  const setAudioLevel = useCallback((lvl: number): void => {
    audioLevelRef.current = lvl;
    audioSubsRef.current.forEach((cb) => cb(lvl));
  }, []);
  const subscribeAudioLevel = useCallback((cb: (lvl: number) => void): (() => void) => {
  ...
  const setLive = useCallback((t: string): void => {
  ...
  const stop = useCallback(
    async ({ silent = false }: { silent?: boolean } = {}): Promise<string> => {
