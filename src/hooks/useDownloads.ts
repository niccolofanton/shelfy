...
// IPC types these channels as `unknown`, so we describe the fields the hook and
// its consumers (the Downloads view) actually read and narrow at the boundary.
export interface DownloadJobRecord {
  ...
  status: string; // 'pending' | 'downloading' | 'done' | 'cancelled' | 'error' | …
  progress: number;
  error?: string | null;
...
// Aggregated download counters surfaced to the UI (mirrors getStats slices).
...
export interface UseDownloads {
  ...
  cancelJob: (key: string) => Promise<void>;
  retryJob: (key: string) => Promise<void>;
}
...
const fallbackKey = (j: DownloadJobRecord): string =>
  `${j.postId}:${j.assetType}:${j.mediaPosition ?? 0}`;

// Narrow the `unknown` IPC progress payload to a job record.
const asJob = (data: unknown): DownloadJobRecord => data as DownloadJobRecord;
...
export function useDownloads(): UseDownloads {
  ...
  const jobsMapRef = useRef<Map<string, DownloadJobRecord>>(new Map());
  ...
  const syncJobs = useCallback(
    (list: DownloadJobRecord[]): void => {
      const map = new Map<string, DownloadJobRecord>();
  ...
  const bumpStats = useCallback((): void => {
    const now = Date.now();
  ...
  const cancelJob = useCallback(async (key: string): Promise<void> => {
    await window.electronAPI.cancelDownloadJob(key);
  }, []);

  const retryJob = useCallback(async (key: string): Promise<void> => {
