...
// The four bookmark kinds a File classifies into.
export type FileKind = 'image' | 'video' | 'pdf' | 'file';

// The wire payload for the `bookmark:add` IPC: original bytes + a preview always
// present (icon as last resort).
...
function canvasToWebpBytes(canvas: HTMLCanvasElement): Promise<Uint8Array | null> {
...
// Pending image-worker request: resolves with the encoded preview bytes.
interface WorkerPending {
  resolve: (bytes: Uint8Array | null) => void;
  reject: (err: Error) => void;
}

// The message shape the image worker posts back.
...
let imgWorker: Worker | false | null = null; // null = non inizializzato, false = non disponibile
let imgWorkerSeq = 0;
const imgWorkerPending = new Map<number, WorkerPending>();
...
function getImageWorker(): Worker | null {
  if (imgWorker === null) {
    ...
    try {
      imgWorker = new Worker(new URL('./imagePreview.worker.ts', import.meta.url), {
        type: 'module',
      });
      imgWorker.onmessage = (e: MessageEvent<ImageWorkerResponse>) => {
        const { id, ok, bytes, error } = e.data || {};
        if (id == null) return;
        const cb = imgWorkerPending.get(id);
        if (!cb) return;
        imgWorkerPending.delete(id);
        if (ok) cb.resolve(bytes ?? null);
...
function imagePreviewViaWorker(file: File): Promise<Uint8Array | null> {
...
async function imagePreviewMainThread(file: File): Promise<Uint8Array | null> {
...
async function imagePreview(file: File): Promise<Uint8Array | null> {
  try {
...
async function videoPreview(file: File): Promise<Uint8Array | null> {
  const url = URL.createObjectURL(file);
...
async function pdfPreview(buf: Uint8Array): Promise<Uint8Array | null> {
...
async function iconPreview(name: string): Promise<Uint8Array | null> {
  const size = 512;
  const canvas = document.createElement('canvas');
  canvas.width = size;
  canvas.height = size;
...
export async function prepareFile(file: File): Promise<PreparedFile> {
  ...
  try {
    ...
    console.warn(
      '[bookmarkFiles] preview failed for',
      file.name,
...
export function previewObjectUrl(bytes: Uint8Array | null | undefined): string | null {
  if (!bytes) return null;
  // `Uint8Array<ArrayBufferLike>` (strict lib) is a valid BlobPart at runtime; the
  // generic buffer param widens to SharedArrayBuffer, which BlobPart's view type rejects.
  return URL.createObjectURL(new Blob([bytes as BlobPart], { type: 'image/webp' }));
}
