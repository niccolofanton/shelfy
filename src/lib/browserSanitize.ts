...
// One sanitized media slide kept on a sanitized item: type coerced to the two
// canonical kinds, url already validated as http(s).
export interface SanitizedMedia {
  type: 'image' | 'video';
  url: string;
}

// A sanitized intercepted item: a bounded, coerced projection of the
// page-controlled payload. The index signature lets callers treat it as the
// loose `{ id, shortcode?, [k]: unknown }` shape they cross the IPC with.
...
export function sanitizeInterceptedItem(it: unknown, platform: string): SanitizedItem | null {
  ...
  for (const f of STR_FIELDS) out[f] = clampStr(rec[f], f === 'timestamp' ? 64 : MAX_URL_LEN);
  out.text = clampStr(rec.text, MAX_TEXT_LEN);
  out.thumbnailUrl = isHttpUrl(rec.thumbnailUrl) ? rec.thumbnailUrl : '';
  const media = Array.isArray(rec.media) ? rec.media : [];
  out.media = media
    .filter(
      (m): m is Record<string, unknown> =>
        !!m && typeof m === 'object' && isHttpUrl((m as Record<string, unknown>).url),
    )
    .slice(0, MAX_MEDIA)
    .map(
      (m): SanitizedMedia => ({
...
export function sanitizeInterceptedBatch(items: unknown[], platform: string): SanitizedItem[] {
  const out: SanitizedItem[] = [];
