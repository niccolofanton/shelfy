...
export const isPostDetail = (u) => /\/(?:p|reel|tv)\/[^/]/.test(u) || /\/pin\/\d+/.test(u);
...
export function parseIgFolder(url) {
...
export function parsePinBoard(url) {
...
export function deslugify(slug, fallback = 'Cartella') {
  const s = String(slug || '')
    .replace(/[-_]+/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
  ...
  return s ? s.replace(/(^|\s)(\p{L})/gu, (_, sep, ch) => sep + ch.toUpperCase()) : fallback;
}
...
export function isAllowedUrl(tab, url) {
  try {
    const u = new URL(url);
...
export function safeUrl(tab, url, fallback) {
  return isAllowedUrl(tab, url) ? url : fallback;
}
