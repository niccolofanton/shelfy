...
// ── Local IPC shapes ────────────────────────────────────────────────────────────
// These channels return `Promise<unknown>` / `unknown[]` at the bridge (their
// runtime shapes live in not-yet-typed main-process modules). We narrow them
// here to exactly the fields this view reads.

// One downloadable model preset (vlm/stt/emb catalog entry).
interface AiModel {
  id: string;
  name: string;
  tier?: string;
...
// Aggregate readiness probe (the object fetchAiSetup builds).
interface AiSetup {
  binaries: { ready?: boolean } | null;
  vlm: AiModel[];
  stt: AiModel[];
  emb: AiModel[];
  complete?: boolean;
...
// getHardwareInfo() → detected hardware + a hardware-fit model recommendation.
interface HardwareSpec {
  cpu?: { physical?: number | null } | null;
  totalRamGB?: number | null;
  gpu?: { name?: string | null } | null;
...
// getVariantState() → the active / recommended acceleration backend.
interface VariantState {
  effective?: string | null;
  recommended?: string | null;
}

// Live per-item install state, layered over the on-disk catalogs.
type ItemStatusKind = 'idle' | 'wait' | 'run' | 'done' | 'error';
interface ItemState {
...
type FamilyKey = 'vlm' | 'stt' | 'engine' | 'emb';
type LiveState = Partial<Record<FamilyKey, ItemState>>;

// Progress events pushed from main (onModelProgress / onBinariesProgress / …).
interface ModelProgressEvent {
  progress?: number;
}
interface BinariesProgressEvent {
  fraction?: number;
  phase?: string;
  error?: string;
}
...
function fmtGB(n: number | null | undefined, lang: string): string | null {
...
interface HwChipProps {
  icon: LucideIcon;
  label: string;
  value: React.ReactNode;
}

function HwChip({ icon: Icon, label, value }: HwChipProps) {
  return (
...
function ItemStatus({ state, sizeText }: ItemStatusProps) {
...
// One step in the sequential install chain.
...
export default function AiOnboarding({ onDone, onSkip, onOpenSettings }: AiOnboardingProps) {
  const t = useT('aiOnboarding');
  const { lang } = useLang();

  const [setup, setSetup] = useState<AiSetup | null>(null); // { binaries, vlm, stt, emb, complete }
  const [hwInfo, setHwInfo] = useState<HardwareInfo | null>(null); // { hardware, recommendedModelId, ... }
  const [variant, setVariant] = useState<VariantState | null>(null); // { effective, recommended, ... }
  const [selectedVlm, setSelectedVlm] = useState<string | null>(null);
  ...
  const refreshAll = async (): Promise<AiSetup | null> => {
    const api = window.electronAPI;
    const safe = <T,>(p: Promise<T> | T | undefined): Promise<T | null> =>
      Promise.resolve(p as Promise<T>).catch(() => null);
    const [s, h, v] = await Promise.all([
      fetchAiSetup() as Promise<AiSetup | null>,
      safe(api?.getHardwareInfo?.() as Promise<HardwareInfo> | undefined),
      safe(api?.getVariantState?.() as Promise<VariantState> | undefined),
    ]);
    if (s) setSetup(s);
    if (h) setHwInfo(h);
    if (v) setVariant(v);
  ...
  useEffect(() => {
    ...
    const track = (key: FamilyKey) => (p: unknown) =>
      setLive((s) => {
        const cur = s[key];
        if (cur && cur.status !== 'run') return s;
        const ev = p as ModelProgressEvent | null | undefined;
        return { ...s, [key]: { ...cur, status: 'run', progress: ev?.progress ?? 0 } };
      });
    const offs = [
      api?.onModelProgress?.(track('vlm')),
      api?.onSttModelProgress?.(track('stt')),
      api?.onEmbModelProgress?.(track('emb')),
      api?.onBinariesProgress?.((p: unknown) =>
        setLive((s) => {
          const prev = s.engine;
          if (prev && prev.status !== 'run') return s;
          const ev = p as BinariesProgressEvent | null | undefined;
          ...
          if (ev?.phase === 'done') return { ...s, engine: { status: 'done', progress: 1 } };
          if (ev?.phase === 'error')
            return { ...s, engine: { ...prev, status: 'error', error: ev?.error } };
          return {
            ...s,
            engine: { ...prev, status: 'run', progress: ev?.fraction ?? 0, phase: ev?.phase },
          };
        }),
      ),
    ];
  ...
  useEffect(() => {
    const onChanged = async (): Promise<void> => {
      ...
      await refreshAll();
      setLive((s) =>
        Object.fromEntries(Object.entries(s).filter(([, v]) => v?.status === 'error')),
      );
    };
  ...
  const vlmModels = useMemo<AiModel[]>(() => setup?.vlm || [], [setup]);
  ...
  const itemState = (key: FamilyKey, ready: boolean, downloading: boolean): ItemState => {
    const cur = live[key];
    if (cur) return cur;
  ...
  const states: Record<FamilyKey, ItemState> = {
    vlm: itemState(
      'vlm',
      vlmReady,
  ...
  const totalMissingGB = useMemo<number>(() => {
    let sum = 0;
    ...
    const plan: InstallStep[] = (
      [
        ...
        !sttReady && sttTarget
          ? {
              key: 'stt',
              run: () => api.sttDownloadModel(sttTarget.id),
              event: 'stt-model-changed',
            }
          : null,
        !engineReady
          ? {
              ...
              run: async () => {
                const r = (await api.ensureBinaries(false)) as {
                  ok?: boolean;
                  error?: string;
                } | null;
        ...
        !embReady && embTarget
          ? {
              key: 'emb',
              run: () => api.embDownloadModel(embTarget.id),
              event: 'emb-model-changed',
            }
          : null,
      ] as (InstallStep | null)[]
    ).filter((s): s is InstallStep => Boolean(s));
  ...
  async function startAnalyzing(): Promise<void> {
    try {
              ...
              <div className="mt-3 flex flex-col gap-2">
                {selModel && (
                  <ModelOption
                    ...
                    ramShortfall={!!hw && (selModel.minRamGB ?? 0) > Math.ceil(hw.totalRamGB ?? 0)}
                  ...
                  <div className="u-fade-in flex flex-col gap-2">
                    {alternatives.map((m) => (
                      <ModelOption
                        ...
                        ramShortfall={!!hw && (m.minRamGB ?? 0) > Math.ceil(hw.totalRamGB ?? 0)}
                        disabled={installing || externalBusy}
