...
const SEED_PROMPT_KEYS = ['seed1', 'seed2', 'seed3', 'seed4', 'seed5', 'seed6'] as const;

// ────────────────────────────────────────────────────────────────────────────
// Local shapes mirroring the module-scope store (useAiSearch) and the dictation
// hook. These hooks convert to TS in parallel; the shapes below match the values
// they actually produce at runtime (see useAiSearch.js / useDictation.js).
// ────────────────────────────────────────────────────────────────────────────

// The store hooks convert to TS in parallel; derive every shape consumed here
// straight from their return types so this view stays assignment-compatible with
// whatever the converted hooks ultimately export (no structural drift).
type AiSearchState = ReturnType<typeof useAiSearch>;
type Actions = AiSearchState['actions'];
type ChatMessage = AiSearchState['messages'][number];
// Two-tier tag grouping + free-text keywords carried by an assistant reply, as
// the assistant message holds it. The fields originate from an IPC `unknown`
// payload, so they're read via guarded narrowing rather than trusted statically.
type ProposedGroups = ChatMessage['proposedGroups'];
type TagMode = AiSearchState['tagMode'];

// Pull a string[] off an open object-ish value only when the key really holds an
// array of strings; otherwise an empty list. Keeps proposedGroups access strict
// without assuming the static type carries broad/specific/keywords.
...
type SourceScope = AiSearchState['source'];

type Dictation = ReturnType<typeof useDictation>;
type VoiceModelStatus = Dictation['modelStatus'];
type VoiceModelProgress = Dictation['modelProgress'];
...
function UserBubble({ content }: UserBubbleProps): React.JSX.Element {
  return (
...
function TagChipRow({ tags, activeTags, onToggleTag }: TagChipRowProps): React.JSX.Element {
  return (
...
interface AssistantBubbleProps {
  ...
  proposedTags?: ChatMessage['proposedTags'];
  ...
  onApply: Actions['applyMessageTags'];
}
...
function AssistantBubble({
  ...
  const broad = stringArrayField(proposedGroups, 'broad');
  const specific = stringArrayField(proposedGroups, 'specific');
  // Free-text keywords extracted from the user message — applied as text filters
  // (sky tone), toggled like tags.
  const keywords = stringArrayField(proposedGroups, 'keywords');
...
function StreamingBubble({ text }: StreamingBubbleProps): React.JSX.Element {
  const t = useT('aiSearch');
  return (
...
interface MessageListProps {
  ...
  activeTags: string[];
  activeKeywords: string[];
  appliedMessageId: AiSearchState['appliedMessageId'];
  onToggleTag: Actions['toggleTag'];
  onToggleKeyword: Actions['toggleKeyword'];
  onApply: Actions['applyMessageTags'];
}
...
interface VoiceModelBannerProps {
  ...
  modelProgress: VoiceModelProgress;
  onDownload: () => void;
}
...
function VoiceModelBanner({
  modelStatus,
  modelProgress,
  onDownload,
}: VoiceModelBannerProps): React.JSX.Element | null {
  const t = useT('aiSearch');
...
export default function AiSearch({
  ...
  const [collectionModal, setCollectionModal] = useState<{ ids: string[] } | null>(null);
  ...
  const logScrollRaf = useRef<number>(0);
  useEffect(() => {
    ...
    <div data-testid="aisearch-view" className="flex h-full overflow-hidden">
      ...
      {activePost && (
        <PostModal
          ...
          onApplyAiFilter={({ tag }: { tag?: string } = {}) => {
          ...
          onReanalyzeWeb={
            onReanalyzeWeb
              ? // PostModal declares onReanalyzeWeb as `() => void`, but it forwards
                // the callback to WebMetaPanel which invokes it with the post
                // (`onReanalyzeWeb(post)`); the declared type is too narrow, so cast
                // the post-taking handler to the prop's signature.
                (((p: Shelfy.Post) => {
                  setActivePost(null);
                  onReanalyzeWeb(p);
                }) as () => void)
