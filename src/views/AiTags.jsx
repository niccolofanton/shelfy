...
function Chip({ label, count, active, onClick, onRemove, color }) {
...
function Dashboard({ overview, onAnalyzeMissing, onCancelAnalyze, analyzing, analyzeActive = 0 }) {
  const t = useT('aiTags');
...
function MergeModal({ suggestions, onClose, onMerge, onRename, busy }) {
  const t = useT('aiTags');
...
export default function AiTags({
  ...
  const [resultsNonce, setResultsNonce] = useState(0); // bump to force a refetch
  ...
  const [aliasProgress, setAliasProgress] = useState(null);
  const { toast, toastClosing, showToast } = useToast();

  const clusterCancelledRef = useRef(false);
  const aliasCancelledRef = useRef(false);
  ...
  const handleRegenerate = useCallback(async () => {
    ...
    try {
      const res = await regenerateClusters((p) => setClusterProgress(p));
      showToast(
        t('proposedClusters', { count: res?.count ?? 0, candidates: res?.candidates ?? 0 }),
      );
    } catch (err) {
      if (clusterCancelledRef.current) showToast(t('regenCancelled'));
      else
        showToast(
          String(err?.message ?? '').includes('MODEL_NOT_READY')
            ? t('modelNotReady')
            : t('regenError'),
        );
    } finally {
  ...
  const handleRenameCluster = useCallback((id, label) => renameCluster(id, label), [renameCluster]);
  const handleRemoveTagFromCluster = useCallback(
    (tag, id) => removeTagFromCluster(tag, id),
    [removeTagFromCluster],
  );
  ...
  const handleProposeAliases = useCallback(async () => {
    ...
    try {
      const res = await proposeAliases((p) => setAliasProgress(p));
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="border-b border-[#2e2e2e] bg-[#0f0f0f] px-3 py-2.5 space-y-2">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {/* Active filter chips */}
              {selectedTags.map((t) => (
                <Chip key={t} label={t} active onRemove={() => removeTag(t)} />
              ))}
              {entity && (
                <Chip
...
function ClusterCard({
  ...
  const [label, setLabel] = useState(clusterName);
  useEffect(() => {
...
function AliasCard({ alias, busy, onAccept, onDismiss }) {
  const t = useT('aiTags');
...
function defaultCollectionName(tags, entity, fallback) {
