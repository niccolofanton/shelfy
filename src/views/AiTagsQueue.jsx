...
function PhaseStepper({ active, pulse }) {
  const t = useT('aiQueue');
...
function JobThumb({ job, px = 48, rounded = 'rounded' }) {
...
function StreamingText({ text, animate }) {
...
const JobRow = React.memo(function JobRow({ job, now, onOpen, onCancel, onRetry }) {
  const t = useT('aiQueue');
  const {
    postId,
    platform,
  ...
  useLayoutEffect(() => {
    ...
    if (!el) return undefined;
    const measure = () => setTagsFadeRight(el.scrollWidth - el.clientWidth > 1);
    measure();
    const ro = new ResizeObserver(measure);
    ro.observe(el);
    return () => ro.disconnect();
  ...
  const elapsedMs = running && job.startedAt ? now - job.startedAt : null;
      ...
      <div className="flex flex-col gap-1 shrink-0 w-[248px] overflow-hidden">
        ...
        <div className="flex items-center gap-2 text-[10px]" style={{ color: 'var(--text-muted)' }}>
          <span className="flex items-center gap-1">
            ...
            {MEDIA_TYPE_KEY[mediaType] ? t(MEDIA_TYPE_KEY[mediaType]) : t('mediaImage')}
          </span>
...
const STATUS_RANK = { extracting: 0, analyzing: 0, pending: 0, error: 1, cancelled: 2, done: 3 };

export default function AiTagsQueue({ onOpenPost }) {
  const t = useT('aiQueue');
  ...
  const summary = useMemo(
    () => analysisSummary(socialJobs, concurrency),
...
function TimingCard({ icon, label, value, highlight = false }) {
  return (
    <div
...
function StatPill({ label, value, icon, alert = false }) {
  return (
    <span
