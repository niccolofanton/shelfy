...
function normPalette(pal) {
  if (!Array.isArray(pal)) return [];
  return pal
    .map((c) => (typeof c === 'string' ? { hex: c, role: '' } : c))
...
function snapshotToView(snap, base) {
  const pages = (Array.isArray(snap.webPages) ? snap.webPages : [])
    .filter((p) => p && p.screenshotPath)
    .map((p) => ({
...
function snapshotToPost(snap, base) {
  return {
    ...(base.post || {}),
...
function EventIcon({ kind, size = 14 }) {
...
// ════════════════════════════════════════════════════════════════════════════
function SegmentedStepper({ phases, active, accent, running = true, labelClass = 'text-[10px]' }) {
...
function PhaseStepper({ status, accent }) {
...
function streamedString(text, key) {
  if (!text) return '';
...
function streamedArray(text, key) {
  if (!text) return [];
  const m = text.match(new RegExp(`"${key}"\\s*:\\s*\\[`));
  if (!m) return [];
  const items = [];
...
function AiAnalysis({ aiJob, post, modelReady, captureDone, accent, onCancel, onRetry }) {
  const t = useT('aiWebsites');
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        >
          ...
          <span>
            {job.stage || (STATUS_KEY[job.status] ? t(STATUS_KEY[job.status]) : job.status)}
          </span>
        </div>
        {isActive && (
          <div
...
// ════════════════════════════════════════════════════════════════════════════
function Timeline({ events, active }) {
  const t = useT('aiWebsites');
  const { lang } = useLang();
...
// ════════════════════════════════════════════════════════════════════════════
function VersionBar({ versions, activeId, onSelect, onDelete }) {
  const t = useT('aiWebsites');
  const { lang } = useLang();
  return (
    <div
              ...
              >
                {p.screenshotPath ? (
                  <img
                    ...
                    src={assetThumbUrl(p.screenshotPath, 480)}
                    alt=""
        ...
        <ImageLightbox
          images={pages.map((p) => ({
            ...
            chunks: Array.isArray(p.chunks)
              ? p.chunks.map((c) => assetUrl(c.screenshotPath)).filter(Boolean)
              : undefined,
            label: (() => {
              try {
...
//  Main view
// ════════════════════════════════════════════════════════════════════════════
export default function AiWebsites({ webJobs, onAddSite, onOpenPost }) {
  const t = useT('aiWebsites');
  const tc = useT('common');
  ...
  useEffect(() => {
    const t = setTimeout(() => setEffectiveQuery(searchQuery.trim()), 250);
  ...
  const { entries, totalEntries } = useMemo(() => {
    ...
    const byKey = new Map(fromPosts.map((e) => [e.key, e]));
    for (const j of jobs) {
  ...
  const aiJob = selected && analysis?.jobFor ? analysis.jobFor(selected.postId) : null;
  ...
  const [snapshotsPostId, setSnapshotsPostId] = useState(null);
  ...
  const activeSnapshot =
    snapshotsValid && activeSnapshotId
      ...
      : null;
  const detailJob = activeSnapshot ? snapshotToView(activeSnapshot, selected) : selected;
  const detailPost = activeSnapshot ? snapshotToPost(activeSnapshot, selected) : selected?.post;
  ...
  const {
    ...
    clearSelection,
    resetAnchor,
  } = useRangeSelect(sorted, (e) => e.postId);
  const sortedRef = useRef(sorted);
  sortedRef.current = sorted;
  const [deleteDialog, setDeleteDialog] = useState(null); // null | { count }
            ...
            {noResults ? (
              ...
              sorted.map((job) => (
                <QueueRow
                  ...
                  snapshotCount={(snapshotCounts[job.postId] || 0) + 1}
                />
              ))
            )}
          </div>
