...
// Concept-combination mode for the AI suggested-tag chips (OR widens, AND narrows).
type ConceptMode = 'or' | 'and';
// Translator returned by useT — namespaced key + optional interpolation vars.
type Translate = (key: string, vars?: Record<string, string | number>) => string;

// The Gallery's UI-side filter state. Mapped to the backend query shape by
// toApiFilters (which strips 'all' sentinels and pagination). 'limit' is the
// infinite-scroll window; everything else mirrors a FilterBar/FilterDrawer control.
...
// Patch handed to PostModal's onApplyAiFilter — a partial set of filter fields
// to merge into the active query.
type FilterPatch = Partial<GalleryFilters>;

// Background source-sync job shape (per platform) reported by useSourceSync.
...
// Source-picker shapes shared with FilterDrawer: `ActiveSource` drives the
// highlight, `SourceSelection` is what a pick reports back (with optional label/
// color). Mirrors FilterDrawer's internal (non-exported) prop types.
...
interface GalleryProps {
  ...
  onCreateCollection?: (input: {
    name: string;
    color: string;
  }) => Promise<Shelfy.Collection | undefined> | Shelfy.Collection | undefined;
  onAssigned?: () => void;
  ...
  sourceSyncJobs?: Record<string, SourceSyncJob | undefined>;
  onSyncSource?: (target: SyncTarget) => void;
...
const SORT_OPTIONS: ReadonlyArray<{ value: SortOrder; labelKey: string }> = [
...
export default function Gallery({
  ...
  const changeConceptMode = useCallback((m: ConceptMode) => {
  ...
  const [assignedOverlay, setAssignedOverlay] = useState<Map<string, Set<number>>>(() => new Map());
  ...
  const dragIndexFromEvent = (e: React.MouseEvent): number | null => {
    const target = e.target instanceof Element ? e.target : null;
    const el = target?.closest('[data-post-index]');
  ...
  const handleAnalyzeSelected = async (): Promise<void> => {
    ...
    try {
      ...
      if (status && typeof status === 'object' && 'ready' in status && !status.ready) {
  ...
  const handleDownloadSuggested = async (): Promise<void> => {
  ...
  const postsById = useMemo(() => {
    const m = new Map<string, Shelfy.Post>();
