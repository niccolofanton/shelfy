...
import { ActivityProvider, type ActivitySources } from '../../src/hooks/useActivity';

// The download/save source jobs in these fixtures carry a couple of extra runtime
// fields (a job `key`, a `platform` on the save record) that buildActivities does
// not read. They are kept verbatim and the shapes narrowed via these aliases so
// the (deliberately wider-than-typed) literals satisfy the source contract.
type DownloadJobsArg = NonNullable<NonNullable<ActivitySources['downloads']>['jobs']>;
type SaveRecordArg = NonNullable<NonNullable<ActivitySources['save']>['lastSave']>;

function setup(
  sources: ActivitySources = {},
  props: Partial<ComponentProps<typeof ActivityCenter>> = {},
) {
...
describe('ActivityCenter', () => {
  ...
  it('navigates to the relevant view when an item body is clicked', () => {
    const { onNavigate } = setup({
      downloads: {
        jobs: [{ key: '1', status: 'downloading', progress: 0.1 }] as unknown as DownloadJobsArg,
      },
    });
    fireEvent.click(screen.getByTestId('activity-strip'));
  ...
  it('navigates from a recent log entry when clicked', () => {
    ...
    rerender(
      <ActivityProvider
        save={{
          active: false,
          lastSave: { count: 2, platform: 'instagram', ts: 9 } as unknown as SaveRecordArg,
        }}
      >
  ...
  it('logs a recent entry when a finished save batch is reported', () => {
    ...
    rerender(
      <ActivityProvider
        save={{
          active: false,
          lastSave: { count: 8, platform: 'instagram', ts: 123 } as unknown as SaveRecordArg,
        }}
      >
        <ActivityCenter onAction={() => {}} onNavigate={() => {}} />
      </ActivityProvider>,
  ...
  function withUpdateState(state: { status: string; version?: string | null }) {
