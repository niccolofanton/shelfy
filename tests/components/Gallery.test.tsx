...
type VirtualPostGridModule = typeof import('../../src/components/VirtualPostGrid');
type VirtualPostGridProps = ComponentProps<VirtualPostGridModule['default']>;
...
const gridProps = vi.hoisted(() => ({ current: null as VirtualPostGridProps | null }));
vi.mock('../../src/components/VirtualPostGrid', async (importOriginal) => {
  const actual = await importOriginal<VirtualPostGridModule>();
  const { createElement } = await import('react');
  return {
    ...actual,
    default: function VirtualPostGridSpy(props: VirtualPostGridProps) {
...
// getPosts resolves the { posts, total } page envelope (PostSearchResult).
const postsPage = (
  posts: Shelfy.Post[],
  total: number,
): { posts: Shelfy.Post[]; total: number } => ({ posts, total });

beforeEach(() => {
  vi.mocked(window.electronAPI.getPosts).mockResolvedValue(postsPage(POSTS, POSTS.length));
});

async function renderGallery(props: Partial<ComponentProps<typeof Gallery>> = {}) {
  const utils = render(
    <Gallery
...
describe('Gallery — bulk actions', () => {
  ...
  it('"Analizza" enqueues only the on-disk ids and suggests downloading the rest', async () => {
    vi.mocked(window.electronAPI.splitForAnalysis).mockResolvedValue({
      analyzable: ['p1'],
      needsDownload: ['p2'],
    });
    await renderGallery();
  ...
  it('"Seleziona tutti" appears when total > loaded and selects all matching ids', async () => {
    ...
    vi.mocked(window.electronAPI.getPosts).mockResolvedValue(postsPage(POSTS, 200));
    vi.mocked(window.electronAPI.getPostIds).mockResolvedValue(['p1', 'p2', 'p3', 'p4']);
  ...
  it('"Seleziona tutti" checks the loaded cards instantly, before the id query resolves', async () => {
    ...
    vi.mocked(window.electronAPI.getPostIds).mockReturnValue(
...
describe('Gallery — quick-select (hover checkbox contract)', () => {
  it('onQuickSelect arms select mode with exactly that post selected', async () => {
    ...
    act(() => gridProps.current?.onQuickSelect?.(POSTS[1], {} as SyntheticEvent));
