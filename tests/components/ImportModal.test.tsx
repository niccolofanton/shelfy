...
function setup(props: Partial<ComponentProps<typeof ImportModal>> = {}) {
  const onClose = vi.fn();
...
describe('ImportModal', () => {
  ...
  describe('idle → file selected', () => {
    it('clicking Choose File calls window.electronAPI.openFile()', async () => {
      vi.mocked(window.electronAPI.openFile).mockResolvedValue(null);
      setup();
    ...
    it('shows filename in code element after file is selected', async () => {
      vi.mocked(window.electronAPI.openFile).mockResolvedValue(
        '/Users/USERNAME/exports/saved-posts.json',
      );
      setup();
      fireEvent.click(screen.getByRole('button', { name: /scegli file/i }));
  ...
  describe('idle → importing', () => {
    it('clicking Import calls window.electronAPI.importJSON with the file path', async () => {
      ...
      vi.mocked(window.electronAPI.openFile).mockResolvedValue(filePath);
      // Keep importJSON pending so we can observe the importing state
      vi.mocked(window.electronAPI.importJSON).mockReturnValue(
        new Promise<Shelfy.ImportResult>(() => {}),
      );
  ...
  describe('importing → done', () => {
    async function setupDoneState() {
      const filePath = '/Users/USERNAME/exports/saved-posts.json';
      vi.mocked(window.electronAPI.openFile).mockResolvedValue(filePath);
      vi.mocked(window.electronAPI.importJSON).mockResolvedValue({
        imported: 5,
      } as unknown as Shelfy.ImportResult);
  ...
  describe('importing → error', () => {
    async function setupErrorState(message = 'Failed to parse JSON') {
      ...
      vi.mocked(window.electronAPI.openFile).mockResolvedValue(filePath);
      vi.mocked(window.electronAPI.importJSON).mockRejectedValue(new Error(message));
