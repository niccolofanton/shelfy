...
// Inject a mock for `electron` into Node's require cache before analyzer.js is
// loaded. analyzer.js requires `electron` (for app.getPath) and `./db`, which in
// turn loads the native better-sqlite3 binding — patching the cache directly is
// the reliable cross-version solution (vi.mock hoisting won't intercept the
...
// ABI guard: analyzer pulls in ./db → better-sqlite3, which may be built for
// Electron's ABI rather than the current Node's. Skip (don't fail) on mismatch.
let analyzer,
  loadErr = null;
try {
  analyzer = req('../../electron/analyzer.js');
} catch (e) {
  loadErr = e;
}
...
if (loadErr) {
  console.warn(
    `[analyzer.test] skipping suites: native module not loadable (${nativeMismatch ? 'ABI mismatch' : String(loadErr)})`,
  );
}
