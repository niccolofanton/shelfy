...
import * as igParser from '../../electron/ig-parser';

// ---------------------------------------------------------------------------
// Local result/input shapes — ig-parser's own types are file-internal, so the
...
interface IgParser {
  parseResponseBody: (data: ResponseBody) => ParseResult;
  normalizeExportedPost: (post: ExportedPost) => ParsedPost;
  igDateFromShortcode: (shortcode: string | null) => string;
}

const { parseResponseBody, normalizeExportedPost, igDateFromShortcode } =
  igParser as unknown as IgParser;
...
describe('parseResponseBody – GraphQL edge format', () => {
  const makeEdge = (node: Record<string, unknown>): { node: Record<string, unknown> } => ({ node });
...
function shortcodeForMs(ms: number): string {
