...
import * as twParser from '../../electron/tw-parser';

// ---------------------------------------------------------------------------
// Local result/input shapes — tw-parser's own types are file-internal, so the
// test mirrors the (stable) public result shape it asserts against. Input
// fixtures are intentionally loose (untrusted API payloads) and passed through
// as the parser's wide param type.
// ---------------------------------------------------------------------------
...
type TweetResultInput = Record<string, unknown>;
type ResponseBody = Record<string, unknown>;
type ExportedPost = Record<string, unknown>;

interface TwParser {
  extractTweet: (result: TweetResultInput | null) => ParsedPost | null;
  parseBookmarkResponse: (data: ResponseBody) => ParseResult;
  normalizeExportedPost: (post: ExportedPost) => ParsedPost;
}

const { extractTweet, parseBookmarkResponse, normalizeExportedPost } =
  twParser as unknown as TwParser;
...
interface MakeTweetOpts {
  restId?: string;
  screenName?: string;
  name?: string;
  fullText?: string;
  createdAt?: string;
  mediaEntities?: TweetMediaEntity[];
}
...
function makeTweetResult({
  ...
  mediaEntities = [],
}: MakeTweetOpts = {}): TweetResultInput {
  return {
    rest_id: restId,
    legacy: {
      id_str: restId,
...
function makeImageMedia(url = 'https://pbs.twimg.com/media/img.jpg'): TweetMediaEntity {
  return { type: 'photo', media_url_https: url };
}

function makeVideoMedia(url = 'https://pbs.twimg.com/media/vid.jpg'): TweetMediaEntity {
  return { type: 'video', media_url_https: url };
}

function makeGifMedia(url = 'https://pbs.twimg.com/media/gif.jpg'): TweetMediaEntity {
  return { type: 'animated_gif', media_url_https: url };
}

// extractTweet returns ParsedPost | null; the tests assert on the non-null case.
// A small helper keeps the assertions readable without repeating non-null casts.
function extract(result: TweetResultInput | null): ParsedPost {
  return extractTweet(result) as ParsedPost;
}
...
describe('extractTweet', () => {
  it('returns a well-formed item for a tweet with a single image', () => {
    ...
    const i = item as ParsedPost;
    expect(i.id).toBe('123456');
    expect(i.platform).toBe('twitter');
    expect(i.postUrl).toBe('https://x.com/alice/status/123456');
    expect(i.profileUrl).toBe('https://x.com/alice');
    expect(i.authorUsername).toBe('alice');
    expect(i.authorName).toBe('Alice A');
    expect(i.text).toBe('Hello');
    expect(i.mediaType).toBe('image');
    expect(i.thumbnailUrl).toBe('https://pbs.twimg.com/media/photo.jpg');
    expect(i.shortcode).toBe('');
  });

  it('detects video mediaType', () => {
    const result = makeTweetResult({ mediaEntities: [makeVideoMedia()] });
    expect(extract(result).mediaType).toBe('video');
  });
  ...
  it('detects multiple images as "images"', () => {
    const result = makeTweetResult({
      mediaEntities: [
        makeImageMedia('https://pbs.twimg.com/media/a.jpg'),
        makeImageMedia('https://pbs.twimg.com/media/b.jpg'),
      ],
    });
    expect(extract(result).mediaType).toBe('images');
  });

  it('returns mediaType "text" for a tweet with no media', () => {
    const result = makeTweetResult({ mediaEntities: [] });
    expect(extract(result).mediaType).toBe('text');
  });
...
describe('parseBookmarkResponse', () => {
  it('parses tweets from bookmark_timeline_v2 format', () => {
    const tweetResult = makeTweetResult({
      restId: 'bm1',
      screenName: 'diane',
      fullText: 'Bookmark v2',
    });
    const data = {
      data: {
...
describe('extractTweet – media array', () => {
  ...
  it('classifies video/gif entries as video in the media array', () => {
    const result = extract(makeTweetResult({ mediaEntities: [makeVideoMedia(), makeGifMedia()] }));
    expect(result.media.map((m) => m.type)).toEqual(['video', 'video']);
  });

  it('produces a single-entry media array for a one-image tweet', () => {
    const result = extract(makeTweetResult({ mediaEntities: [makeImageMedia()] }));
    expect(result.mediaType).toBe('image');
    expect(result.media).toHaveLength(1);
  });

  it('produces an empty media array for a text-only tweet', () => {
    const result = extract(makeTweetResult({ mediaEntities: [] }));
    expect(result.media).toEqual([]);
  });
});
