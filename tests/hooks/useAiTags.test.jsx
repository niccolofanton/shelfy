...
import { useAiTags } from '../../src/hooks/useAiTags.js';
...
describe('useAiTags — initial load', () => {
  it('loads overview/tagStats/clusters/entityStats/health/mergeSuggestions in parallel on mount', async () => {
    window.electronAPI.getAiOverview.mockResolvedValue({ total: 10, analyzed: 4 });
    window.electronAPI.getTagStats.mockResolvedValue([{ tag: 'a', count: 3 }]);
    window.electronAPI.getTagClusters.mockResolvedValue([{ id: 'c1' }]);
    window.electronAPI.getEntityStats.mockResolvedValue([{ entity: 'e1' }]);
    window.electronAPI.getTagHealth.mockResolvedValue({ orphanTags: [], rareTags: 2 });
    window.electronAPI.getTagMergeSuggestions.mockResolvedValue([{ from: 'x', to: 'y' }]);
  ...
  it('defaults array fields to [] and object fields to null when responses are non-array/falsy', async () => {
    window.electronAPI.getAiOverview.mockResolvedValue(null);
    window.electronAPI.getTagStats.mockResolvedValue(null);
    window.electronAPI.getTagClusters.mockResolvedValue(undefined);
    window.electronAPI.getEntityStats.mockResolvedValue(null);
    window.electronAPI.getTagHealth.mockResolvedValue(null);
    window.electronAPI.getTagMergeSuggestions.mockResolvedValue(null);
...
describe('useAiTags — error handling', () => {
  it('sets error to the message when a load call rejects', async () => {
    window.electronAPI.getAiOverview.mockRejectedValue(new Error('boom'));
  ...
  it('sets a fallback error message when rejection has no message', async () => {
    window.electronAPI.getAiOverview.mockRejectedValue(null);
...
describe('useAiTags — refresh()', () => {
  it('reloads all aggregate data', async () => {
    ...
    const before = window.electronAPI.getAiOverview.mock.calls.length;

    await act(async () => { await result.current.refresh(); });

    expect(window.electronAPI.getAiOverview.mock.calls.length).toBeGreaterThan(before);
  });
});
...
describe('useAiTags — renameTag', () => {
  it('calls electronAPI.renameTag then reloads', async () => {
    window.electronAPI.renameTag.mockResolvedValue({ updated: 3 });
    ...
    let res;
    await act(async () => { res = await result.current.renameTag('old', 'new'); });
...
describe('useAiTags — mergeTags', () => {
  it('calls electronAPI.mergeTags then reloads', async () => {
    window.electronAPI.mergeTags.mockResolvedValue({ updated: 5 });
    ...
    let res;
    await act(async () => { res = await result.current.mergeTags(['a', 'b'], 'c'); });
...
describe('useAiTags — analyzeMissing', () => {
  it('calls electronAPI.analyzeMissing and does NOT reload', async () => {
    window.electronAPI.analyzeMissing.mockResolvedValue({ queued: 7 });
    ...
    let res;
    await act(async () => { res = await result.current.analyzeMissing(); });

    expect(window.electronAPI.analyzeMissing).toHaveBeenCalled();
    expect(res).toEqual({ queued: 7 });
    expect(window.electronAPI.getAiOverview.mock.calls.length).toBe(before);
  });
});
...
describe('useAiTags — read helpers', () => {
  it('getPostIdsByTags delegates to electronAPI with tags and mode', async () => {
    window.electronAPI.getPostIdsByTags.mockResolvedValue(['p1', 'p2']);

    const { result } = renderHook(() => useAiTags());
    await waitFor(() => expect(result.current.loading).toBe(false));

    let ids;
    await act(async () => { ids = await result.current.getPostIdsByTags(['a'], 'and'); });
  ...
  it('getTagCooccurrence delegates to electronAPI with tag and limit', async () => {
    window.electronAPI.getTagCooccurrence.mockResolvedValue([{ tag: 'b', count: 2 }]);

    const { result } = renderHook(() => useAiTags());
    await waitFor(() => expect(result.current.loading).toBe(false));

    let res;
    await act(async () => { res = await result.current.getTagCooccurrence('a', 5); });
  ...
  it('fetchPosts calls getPosts with the filters and returns the posts payload', async () => {
    ...
    window.electronAPI.getPosts.mockResolvedValue(payload);
    ...
    let res;
    await act(async () => { res = await result.current.fetchPosts(filters); });
