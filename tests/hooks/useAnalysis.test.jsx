...
describe('useAnalysis — initial load', () => {
  it('calls getAnalyzeStatus and getModelStatus on mount', async () => {
    window.electronAPI.getAnalyzeStatus.mockResolvedValue([]);
    window.electronAPI.getModelStatus.mockResolvedValue({ ready: true, downloading: false });
  ...
  it('populates jobs and modelStatus from API responses', async () => {
    ...
    window.electronAPI.getAnalyzeStatus.mockResolvedValue(fakeJobs);
    window.electronAPI.getModelStatus.mockResolvedValue(fakeModel);
  ...
  it('defaults jobs to [] when getAnalyzeStatus resolves null', async () => {
    window.electronAPI.getAnalyzeStatus.mockResolvedValue(null);
...
describe('useAnalysis — onAnalyzeProgress new job', () => {
  it('adds a new entry to jobs when key is unknown', async () => {
    ...
    window.electronAPI.onAnalyzeProgress.mockImplementation((cb) => {
      capturedCallback = cb;
      return () => {};
    });
...
describe('useAnalysis — onAnalyzeProgress update existing job', () => {
  it('updates the matching job by key rather than adding a duplicate', async () => {
    ...
    window.electronAPI.getAnalyzeStatus.mockResolvedValue([existing]);
    ...
    act(() => { capturedCallback({ key: 'p1:analyze', status: 'done', progress: 1 }); });

    expect(result.current.jobs).toHaveLength(1);
    expect(result.current.jobs[0].status).toBe('done');
    expect(result.current.jobs[0].progress).toBe(1);
  });
  ...
  it('adds a new entry when key differs', async () => {
    ...
    act(() => { capturedCallback({ key: 'p2:analyze', status: 'queued', progress: 0 }); });
...
describe('useAnalysis — jobFor', () => {
  it('returns the job whose key matches `${postId}:analyze`', async () => {
    ...
    window.electronAPI.getAnalyzeStatus.mockResolvedValue(jobs);
  ...
  it('returns null when no job matches the postId', async () => {
    window.electronAPI.getAnalyzeStatus.mockResolvedValue([{ key: 'p1:analyze', status: 'done' }]);
...
describe('useAnalysis — actions', () => {
  it('analyzePost calls electronAPI.analyzePost with the postId', async () => {
    ...
    act(() => { result.current.analyzePost('p1'); });
  ...
  it('cancelJob calls electronAPI.cancelAnalyzeJob with the key', async () => {
    ...
    act(() => { result.current.cancelJob('p1:analyze'); });
  ...
  it('retryJob calls electronAPI.retryAnalyzeJob with the key', async () => {
    ...
    act(() => { result.current.retryJob('p1:analyze'); });
  ...
  it('refreshModel calls electronAPI.getModelStatus again and updates modelStatus', async () => {
    window.electronAPI.getModelStatus.mockResolvedValue({ ready: true });

    const { result } = renderHook(() => useAnalysis());
    await waitFor(() => expect(result.current.modelStatus).toEqual({ ready: true }));

    window.electronAPI.getModelStatus.mockResolvedValue({ ready: false });
    await act(async () => { await result.current.refreshModel(); });
  ...
  it('downloadModel calls electronAPI.downloadModel then refreshes the model', async () => {
    ...
    const modelCallsBefore = window.electronAPI.getModelStatus.mock.calls.length;
    await act(async () => { await result.current.downloadModel(); });

    expect(window.electronAPI.downloadModel).toHaveBeenCalled();
    expect(window.electronAPI.getModelStatus.mock.calls.length).toBeGreaterThan(modelCallsBefore);
  });
});
...
describe('useAnalysis — onModelProgress', () => {
  it('updates modelProgress while downloading (progress < 1)', async () => {
    let capturedCallback;
    window.electronAPI.onModelProgress.mockImplementation((cb) => {
      capturedCallback = cb;
      return () => {};
    });
    ...
    act(() => { capturedCallback({ progress: 0.5, label: 'modello' }); });
  ...
  it('clears modelProgress and refreshes the model when progress >= 1', async () => {
    ...
    const modelCallsBefore = window.electronAPI.getModelStatus.mock.calls.length;
    await act(async () => { capturedCallback({ progress: 1, label: 'modello' }); });
...
describe('useAnalysis — cleanup on unmount', () => {
  it('calls both unsub functions on unmount', () => {
    ...
    window.electronAPI.onAnalyzeProgress.mockReturnValue(unsubJob);
    window.electronAPI.onModelProgress.mockReturnValue(unsubModel);
