...
import useBrowserSync from '../../src/hooks/useBrowserSync.js';
...
describe('useBrowserSync — generation token', () => {
  ...
  it('mirrors step counters into syncCountsRefs synchronously with the save', async () => {
    window.electronAPI.saveInterceptedPosts.mockResolvedValue({ inserted: 2, skipped: 1 });
    const { result } = setup();
    act(() => {
      result.current.startSync('twitter');
    });
