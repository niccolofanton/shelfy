...
import { useDownloads } from '../../src/hooks/useDownloads.js';
...
describe('useDownloads — initial load', () => {
  it('calls getDownloadStatus and getStats on mount', async () => {
    ...
    window.electronAPI.getStats.mockResolvedValue({
      total: 5,
  ...
  it('populates jobs and stats from API responses', async () => {
    ...
    window.electronAPI.getDownloadStatus.mockResolvedValue(fakeJobs);
    window.electronAPI.getStats.mockResolvedValue({
      total: 10,
  ...
  it('defaults stats fields to 0 when downloaded sub-keys are missing', async () => {
    window.electronAPI.getStats.mockResolvedValue({ total: 2 });
...
describe('useDownloads — onDownloadProgress update existing job', () => {
  it('updates the matching job rather than adding a duplicate', async () => {
    ...
    window.electronAPI.getDownloadStatus.mockResolvedValue([existingJob]);
...
describe('useDownloads — progress event coalescing', () => {
  ...
  it('coalesces getStats for a burst of done events into leading + trailing calls', async () => {
    ...
    const callsAfterMount = window.electronAPI.getStats.mock.calls.length;
    ...
    expect(window.electronAPI.getStats.mock.calls.length).toBe(callsAfterMount + 1);
    ...
    expect(window.electronAPI.getStats.mock.calls.length).toBe(callsAfterMount + 2);
  });
});
...
describe('useDownloads — periodic stats refresh', () => {
  ...
  it('polls getStats every 5 seconds while the queue is active', async () => {
    window.electronAPI.getDownloadStatus.mockResolvedValue([activeJob]);
    ...
    const callsBefore = window.electronAPI.getStats.mock.calls.length;
    ...
    expect(window.electronAPI.getStats.mock.calls.length).toBeGreaterThan(callsBefore);
  });
  ...
  it('polls getStats over multiple 5s intervals while the queue is active', async () => {
    ...
    expect(window.electronAPI.getStats.mock.calls.length).toBeGreaterThanOrEqual(callsBefore + 3);
  });
  ...
  it('does NOT poll getStats when the queue is idle (empty)', async () => {
    ...
    expect(window.electronAPI.getStats.mock.calls.length).toBe(callsBefore);
  });
});
...
describe('useDownloads — cleanup on unmount', () => {
  it('calls unsub() when unmounted', () => {
    const unsub = vi.fn();
    window.electronAPI.onDownloadProgress.mockReturnValue(unsub);
    window.electronAPI.getDownloadStatus.mockResolvedValue([]);
  ...
  it('stops calling getStats after unmount (interval cleared)', async () => {
    ...
    unmount();
    const callsAtUnmount = window.electronAPI.getStats.mock.calls.length;
    ...
    expect(window.electronAPI.getStats.mock.calls.length).toBe(callsAtUnmount);
