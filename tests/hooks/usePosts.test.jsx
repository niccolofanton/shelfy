...
describe('usePosts — initial fetch', () => {
  it('calls getPosts with correct apiFilters and populates posts', async () => {
    ...
    window.electronAPI.getPosts.mockResolvedValue({ posts: fakePosts, total: 2 });
...
describe('usePosts — filter conversion', () => {
  it('converts "all" platform and mediaType to undefined', async () => {
    window.electronAPI.getPosts.mockResolvedValue({ posts: [], total: 0 });
    ...
    const [called] = window.electronAPI.getPosts.mock.calls[0];
    expect(called.platform).toBeUndefined();
    expect(called.mediaType).toBeUndefined();
  });
  ...
  it('passes through a specific platform value', async () => {
    ...
    const [called] = window.electronAPI.getPosts.mock.calls[0];
    expect(called.platform).toBe('instagram');
    expect(called.mediaType).toBeUndefined();
  });
...
describe('usePosts — total from backend', () => {
  it('exposes the real total returned by the backend', async () => {
    ...
    window.electronAPI.getPosts.mockResolvedValue({ posts: fakePosts, total: 200 });
  ...
  it('keeps total stable across limit bumps (reflects DB count, not page size)', async () => {
    ...
    window.electronAPI.getPosts.mockResolvedValue({ posts: fakePosts, total: 12 });
...
describe('usePosts — error handling', () => {
  it('sets error to the message when getPosts rejects', async () => {
    window.electronAPI.getPosts.mockRejectedValue(new Error('DB error'));
  ...
  it('sets a fallback error message when rejection has no message', async () => {
    window.electronAPI.getPosts.mockRejectedValue(null);
...
describe('usePosts — category/contentType/tag filters', () => {
  ...
  it('leaves category/contentType/tag undefined when not provided', async () => {
    ...
    expect(called.contentType).toBeUndefined();
    expect(called.tag).toBeUndefined();
  });

  it('re-fetches when category changes', async () => {
    window.electronAPI.getPosts.mockResolvedValue({ posts: [], total: 0 });

    const { result, rerender } = renderHook((props) => usePosts(props), {
...
describe('usePosts — onNewPosts subscription', () => {
  ...
  it('calls the unsub function returned by onNewPosts on unmount', () => {
    const unsub = vi.fn();
    window.electronAPI.onNewPosts.mockReturnValue(unsub);
  ...
  it('triggers a reload after 500ms debounce when onNewPosts fires', async () => {
    ...
    window.electronAPI.onNewPosts.mockImplementation((cb) => {
      capturedCallback = cb;
      return () => {};
    });
    ...
    expect(window.electronAPI.getPosts.mock.calls.length).toBe(callsBefore);
...
describe('usePosts — append-only pagination', () => {
  it('fetches only the missing page (offset = loaded) when limit grows', async () => {
    ...
    window.electronAPI.getPosts.mockResolvedValueOnce({ posts: firstPage, total: 200 });
    ...
    window.electronAPI.getPosts.mockResolvedValueOnce({ posts: secondPage, total: 200 });
  ...
  it('deduplicates by id rows re-served by a shifted offset window', async () => {
    ...
    window.electronAPI.getPosts.mockResolvedValueOnce({ posts: firstPage, total: 10 });
    ...
    window.electronAPI.getPosts.mockResolvedValueOnce({
      posts: [{ id: 'p2' }, { id: 'p3' }],
      total: 10,
    });
  ...
  it('re-fetches from offset 0 when a filter changes', async () => {
    const firstPage = Array.from({ length: 50 }, (_, i) => ({ id: `a${i}` }));
    window.electronAPI.getPosts.mockResolvedValue({ posts: firstPage, total: 200 });
...
describe('usePosts — reload reconciliation', () => {
  it('reuses the previous object identity for unchanged rows', async () => {
    ...
    window.electronAPI.getPosts.mockResolvedValueOnce({ posts: original, total: 2 });
...
describe('usePosts — single-post patch on done events', () => {
  it('patches just the completed post via getPostsByIds, without refetching the list', async () => {
    let downloadCb;
    window.electronAPI.onDownloadProgress.mockImplementation((cb) => {
      downloadCb = cb;
      return () => {};
    });
    const original = [
    ...
    window.electronAPI.getPosts.mockResolvedValue({ posts: original, total: 2 });

    const { result } = renderHook(() => usePosts(defaultFilters));
    await waitFor(() => expect(result.current.posts).toHaveLength(2));
    const callsBefore = window.electronAPI.getPosts.mock.calls.length;

    window.electronAPI.getPostsByIds.mockResolvedValue([{ id: 'p1', videoPath: '/x.mp4' }]);
    await act(async () => {
...
describe('usePosts — inactive view', () => {
  it('accumulates a dirty flag while inactive and reloads once on reactivation', async () => {
    ...
    const { rerender } = renderHook((props) => usePosts(defaultFilters, props), {
    ...
    expect(window.electronAPI.getPosts.mock.calls.length).toBe(callsBefore + 1);
