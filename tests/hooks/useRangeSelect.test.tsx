...
import { useRangeSelect } from '../../src/hooks/useRangeSelect';
...
const getId = (p: Item): string | null => p.id;

function setup(items: Item[] = ITEMS) {
  return renderHook(({ list }: { list: Item[] }) => useRangeSelect<Item, string>(list, getId), {
...
describe('useRangeSelect — shift range', () => {
  ...
  it('skips entries whose getId is nullish (live-only rows without a postId)', () => {
    const items: Item[] = [{ id: 'a' }, { id: null }, { id: 'c' }];
