...
import { buildSyncSteps, type SyncStep } from '../../src/hooks/useSourceSync';
...
describe('buildSyncSteps', () => {
  it('plans Instagram platform sync as all-posts followed by every native folder', () => {
    ...
    expect((steps[1] as Extract<SyncStep, { type: 'ig-folder' }>).collection.id).toBe(1);
    expect((steps[2] as Extract<SyncStep, { type: 'ig-folder' }>).collection.id).toBe(2);
  });
  ...
  it('refuses custom folders and unknown targets', () => {
    expect(buildSyncSteps({ type: 'collection', collectionId: 3 }, collections)).toEqual([]);
    expect(
      buildSyncSteps({ type: 'platform', platform: 'web' as 'instagram' }, collections),
    ).toEqual([]);
