...
import { useWebJobs } from '../../src/hooks/useWebJobs';
...
beforeEach(() => {
  ...
  window.electronAPI.onWebProgress = vi.fn((cb: ProgressCallback) => {
    capturedCallback = cb;
    return () => {};
  });
...
const doneJob = (id: string) => ({ postId: id, status: 'done', url: `https://${id}.test` });

// ─── Initial snapshot ─────────────────────────────────────────────────────────

describe('useWebJobs — initial snapshot', () => {
  it('loads jobs from getWebStatus on mount', async () => {
    vi.mocked(window.electronAPI.getWebStatus).mockResolvedValue([
...
describe('useWebJobs — completed jobs pruning', () => {
  it('caps done/cancelled jobs at 200 when the queue is idle, dropping the oldest', async () => {
    ...
    vi.mocked(window.electronAPI.getWebStatus).mockResolvedValue(snapshot);
