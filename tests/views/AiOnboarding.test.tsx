...
// A single catalog row mirroring the listModels() shape from
// analyzer/stt/embeddings. The bridge types these as `unknown[]`.
...
const vlmCatalog = (patch: Record<string, Partial<CatalogModel>> = {}): CatalogModel[] => [
  {
    id: 'qwen3vl-4b',
    name: 'Qwen3-VL 4B',
    tier: 'Veloce',
...
const sttCatalog = (ready = false): CatalogModel[] => [
  {
    id: 'whisper-turbo-q5',
...
const embCatalog = (ready = false): CatalogModel[] => [
  {
    id: 'e5-small',
...
function mockIncomplete(): void {
  vi.mocked(window.electronAPI.listModels).mockResolvedValue(vlmCatalog());
  vi.mocked(window.electronAPI.sttListModels).mockResolvedValue(sttCatalog());
  vi.mocked(window.electronAPI.embListModels).mockResolvedValue(embCatalog());
  vi.mocked(window.electronAPI.getBinariesStatus).mockResolvedValue({ ready: false, missing: 4 });
}
...
function mockComplete(): void {
  vi.mocked(window.electronAPI.listModels).mockResolvedValue(
    vlmCatalog({ 'qwen3vl-8b': { ready: true } }),
  );
  vi.mocked(window.electronAPI.sttListModels).mockResolvedValue(sttCatalog(true));
  vi.mocked(window.electronAPI.embListModels).mockResolvedValue(embCatalog(true));
  vi.mocked(window.electronAPI.getBinariesStatus).mockResolvedValue({ ready: true, missing: 0 });
}
...
describe('AiOnboarding — wizard iniziale', () => {
  ...
  it('un errore ferma la catena e offre il retry', async () => {
    vi.mocked(window.electronAPI.downloadModel).mockRejectedValue(new Error('rete giù'));
    render(<AiOnboarding />);
    await waitFor(() => expect(screen.getByTestId('ai-onb-install')).toBeInTheDocument());
    fireEvent.click(screen.getByTestId('ai-onb-install'));
