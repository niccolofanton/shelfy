...
const FULL_OVERVIEW: Shelfy.AiOverview = {
  total: 100,
  analyzed: 80,
  unanalyzed: 20,
  uniqueTags: 12,
...
const CLUSTERS = [
  { id: 'c1', topTag: 'food', postCount: 40, tags: ['pasta', 'pizza', 'recipe'] },
] as unknown as Shelfy.TagCluster[];

const ENTITIES: Shelfy.Entity[] = [
...
function populate(): void {
  vi.mocked(window.electronAPI.getAiOverview).mockResolvedValue(FULL_OVERVIEW);
  vi.mocked(window.electronAPI.getTagStats).mockResolvedValue(TAG_STATS);
  vi.mocked(window.electronAPI.getTagClusters).mockResolvedValue(CLUSTERS);
  vi.mocked(window.electronAPI.getEntityStats).mockResolvedValue(ENTITIES);
  vi.mocked(window.electronAPI.getTagHealth).mockResolvedValue(HEALTH);
}
