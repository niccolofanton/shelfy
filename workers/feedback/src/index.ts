...
// ── Bindings letti dal Worker (vedi wrangler.jsonc) ──────────────────────────
// Sottoinsiemi minimali e reali delle binding nativa di Workers che usiamo qui,
// così da restare type-safe senza dipendere da @cloudflare/workers-types.
...
interface Env {
  // Livello 1 — rate-limit per IP (anti-burst).
  PER_IP?: RateLimitBinding;
  // Livello 2 — budget giornaliero GLOBALE in KV.
  BUDGET?: KVNamespaceBinding;
  // Secret server-side: chiave Resend.
  RESEND_API_KEY?: string;
  // Config in chiaro (vars).
  FROM?: string;
  TO?: string;
  DAILY_CAP?: string;
}

// Allegato grezzo in arrivo dall'app (UNTRUSTED) e forma normalizzata per Resend.
...
function sanitizeAttachments(attachments: unknown): ResendAttachment[] {
  if (!Array.isArray(attachments)) return [];
  const out: ResendAttachment[] = [];
  let total = 0;
  for (const a of attachments as IncomingAttachment[]) {
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const data = (body ?? {}) as { message?: unknown; version?: unknown; attachments?: unknown };
    const text = (typeof data.message === 'string' ? data.message : '').trim();
    const version = typeof data.version === 'string' ? data.version.slice(0, 64) : '';
    const files = sanitizeAttachments(data.attachments);
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Errore di rete verso Resend.';
